...
`scripts/lynx-stack-docs-dirs.txt` (`rspeedy/config`, `rspeedy/plugins`,
`rspeedy/api`, `react/api` and `packages`, in both locales) are owned by that
sync: change those pages in lynx-stack, and the daily `Update API docs`
workflow brings them here. A pull request that edits them directly fails the
`Lynx Stack Docs Ownership` check. Old `/api/rspeedy`, `/api/react` and
testing-library URLs are redirected by `docs/public/_redirects`.
