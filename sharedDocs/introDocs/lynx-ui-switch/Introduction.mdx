...
Use a controlled Switch (`checked` + `onChange`) to drive page theme, and let token changes update the Switch visuals automatically. The example also shows a composed `ThemedSwitch` wrapper alongside part-level primitives; see [Styling & Theming](/ui/styling-theming.html).
