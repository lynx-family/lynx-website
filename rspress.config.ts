...
const BUILD_PHASE = process.env.RSPRESS_BUILD_PHASE ?? 'all';
const IS_SITE_BUILD = BUILD_PHASE !== 'llms';
const IS_LLMS_BUILD = BUILD_PHASE !== 'site';
const IS_SPLIT_LLMS_BUILD = BUILD_PHASE === 'llms';
const IS_LIGHTWEIGHT_BUILD =
  process.env.RSPRESS_LIGHTWEIGHT_BUILD === 'true' ||
  NETLIFY_CONTEXT === 'deploy-preview';
const ENABLE_LLMS = IS_LLMS_BUILD && !IS_LIGHTWEIGHT_BUILD;

export default defineConfig({
  root: path.join(__dirname, 'docs'),
  outDir: IS_SPLIT_LLMS_BUILD ? '.rspress-llms-build' : 'doc_build',
  route: {
    exclude: [
      'lynx-compat-data/**/*',
  ...
  themeConfig: {
    // A split site build still ships the same LLM controls even though the
    // Markdown assets are generated by the following, isolated build phase.
    ...(BUILD_PHASE === 'site' && !IS_LIGHTWEIGHT_BUILD
      ? { llmsUI: true }
      : {}),
    editLink: {
      docRepoBaseUrl:
  ...
  plugins: [
    ...
    ...(IS_SITE_BUILD && !IS_LIGHTWEIGHT_BUILD
      ? [
          pluginSitemap({
            siteUrl: PUBLISH_URL,
          }),
      ...
      : []),
    ...(ENABLE_LLMS ? [pluginLLMsPostprocess()] : []),
    pluginAlgolia({
      verificationContent: '6AD08DFB25B7234D',
    }),
    ...(IS_SITE_BUILD ? [pluginSanitizeGeneratedHtml()] : []),
  ],
  markdown: {
