...
| [`headers`](#server-headers) | `Record<string, string \| string[]>` | Adds headers to all responses. |
...
| [`proxy`](#server-proxy) | `ProxyConfig` | Configure proxy rules for the dev server or preview server to proxy requests to the specified service. |
