---
api: lynx-native-api/lynx-service/lynx-service
---
...
Lynx provides a default implementation of Lynx Service, which can be directly [accessed](/guide/start/quick-start?approach=building-block)
...
Implement a concrete Desktop service class. The following example uses `LynxHttpService`; `LynxSecurityService` and `LynxEventReporterService` follow the same pattern and override their own abstract entrypoints.

<CodeFold toggle>

```cpp title="YourHttpService.h"
#include "lynx/platform/embedder/public/lynx_http_service.h"

class YourHttpService : public lynx::pub::LynxHttpService {
 public:
...
Use the corresponding abstract class for each service type:

- `LynxHttpService`: override `Request(...)`
- `LynxSecurityService`: override `VerifyTASM(...)`
- `LynxEventReporterService`: override `OnEvent(...)`
...
If you use your own implementation of Lynx Service, please delete the [default implementation](/guide/start/quick-start.html?approach=building-block&integrate-platform=ios) provided by Lynx.
...
Replace the implementation of log service in `LynxServiceCenter` in the [integration documentation](/guide/start/quick-start.html?approach=building-block&integrate-platform=android) with your implementation.
...
Replace the implementation of log service in `LynxServiceCenter` in the [integration documentation](/guide/start/quick-start.html?approach=building-block&integrate-platform=harmony) with your implementation.
...
Register the implemented service through `LynxServiceCenter`:

```cpp
auto service = std::make_shared<YourHttpService>();
lynx::pub::LynxServiceCenter::GetInstance().RegisterService(service);
```

The registered service type is determined by the concrete service class:

- `LynxHttpService` -> `kServiceTypeHttp`
- `LynxSecurityService` -> `kServiceTypeSecurity`
- `LynxEventReporterService` -> `kServiceTypeEventReporter`
