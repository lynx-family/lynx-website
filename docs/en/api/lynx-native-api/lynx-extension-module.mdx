...
- `OnLynxViewCreate(lynx_view_t* lynx_view)`: called on the UI thread when a `LynxView` instance is created.
...
The extension-module pattern is useful when a native view should always be installed together with the extension itself:
...
class CEFExtensionModule : public lynx::pub::LynxExtensionModule {
 public:
  void OnLynxViewCreate(lynx_view_t* lynx_view) override {
    if (auto* view = lynx::pub::LynxView::Unwrap(lynx_view); view != nullptr) {
