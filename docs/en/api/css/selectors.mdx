...
By default, Lynx does not support `!important`. However, **since engineVersion 3.9**, you can enable it by setting the `engineVersion` to `'3.9'` or higher in your plugin options.

When enabled, `!important` gives a declaration higher priority than non-`!important` ones. If multiple `!important` declarations apply to the same element, the winner is still determined by specificity and source order. For more details, refer to the [MDN documentation on !important](https://developer.mozilla.org/en-US/docs/Web/CSS/important).
