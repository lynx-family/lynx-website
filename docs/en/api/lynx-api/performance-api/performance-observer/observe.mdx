---
api: lynx-api/performance-api/performance-observer/observe
---
...
including [`MetricFcpEntry`](/api/lynx-api/performance-api/performance-entry/metric-fcp-entry) and [`MetricActualFmpEntry`](/api/lynx-api/performance-api/performance-entry/metric-actual-fmp-entry). Alternatively, you can use `"metric.fcp"` to listen only to the `MetricFcpEntry` performance event.
...
:::tip
It is recommended to call this method during initialization, such as in the constructor of a root component or within the [`useMemo`](/api/react/hooks#built-in-react-hooks) Hook,
...
<Go
  ...
  defaultFile="src/performance-observer-observe/index.tsx"
