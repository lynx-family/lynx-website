---
tag: BTS
api: lynx-api/performance-api/performance-metric
---
...
In Lynx, several metric types are provided, such as [`MetricFcpEntry`](./performance-entry/metric-fcp-entry) and [`MetricActualFmpEntry`](./performance-entry/metric-actual-fmp-entry). Each metric type contains multiple metrics based on different starting points (for example, `MetricFcpEntry` includes metrics like `fcp`, `lynxFcp`, and `totalFcp`). To better describe the duration and timestamps of each metric, the `PerformanceMetric` type is designed.
