---
api: lynx-api/performance-api/performance-entry/metric-fcp-entry
---

import { APISummary, APITable, Go, RuntimeBadge, Deprecated } from '@lynx';
...
FCP is a key performance metric that measures the time taken for the [first-frame rendering](/guide/spec#first-frame-rendering-or-ffr) of lynx app to complete. It refers to the time taken for users to see any content (such as text or images) on a lynx app for the first time. `MetricFcpEntry` interface provides timing information about this metric, inheriting from [`PerformanceEntry`](../performance-entry).
...
<Go
  ...
  defaultFile="src/fcp-entry/index.tsx"
  defaultEntryFile="dist/fcp-entry.lynx.bundle"
