...
:::warning
This event has been deprecated since Lynx SDK 3.7. Related data can be obtained from [`LoadBundleEntry`](/api/lynx-api/performance-api/performance-entry/load-bundle-entry) / [`ReloadBundleEntry`](/api/lynx-api/performance-api/performance-entry/reload-bundle-entry).
:::

<APISummary />

The `InitContainerEntry` interface documents key moments related for the [container](/guide/spec#container) preparation work before the Lynx page is displayed. It inherits from [`PerformanceEntry`](/api/lynx-api/performance-api/performance-entry).
