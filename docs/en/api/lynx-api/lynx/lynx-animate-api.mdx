...
After **Lynx version 3.3**, the `Animate()` can be called via the `createSelectorQuery()`, you can chain chain multiple animation with it.
...
- Before Lynx version 3.3, the Animate API only supported creating one animation. Therefore, if `lynx.getElementById("test").animate` is called multiple times on a single node before Lynx version 3.3, the last created animation will overwrite the previous ones.
...
- Before Lynx version 3.3, the Animate API only supported the getElementById API. After Lynx version 3.3, it supports the SelectQuery API.
