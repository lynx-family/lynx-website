...
`<svg>` is the cornerstone element for embedding Scalable Vector Graphics (SVG) directly within your markup, from simple icons to complex data visualizations, that scale flawlessly across every screen size and pixel density. Instead of creating individual element nodes on the main thread for each SVG tag, the `<svg>` element offloads SVG parsing to a background thread and renders the entire graphic as a single native view, significantly reducing main thread overhead in both parsing time and element node count. This makes it optimized for performant, static rendering of vector graphics rather than per-node interaction or animation. It covers 17 commonly used SVG tags and over 40 attributes.
