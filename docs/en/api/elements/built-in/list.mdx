...
**2. Set scroll direction and layout form**: Set the attribute [`scroll-orientation`](#required-scroll-orientation) to specify the layout and scroll direction, and set [`list-type`](#required-list-type) and [`span-count`](#required-span-count) to specify the layout form.

**3. Configure child nodes**: Use the `<list-item>` tag as a direct child node of `<list>` , and set [`item-key`](#required-item-key) and [`key`](#required-key) for `<list-item>`, ensuring they are consistent.
...
**1. Set layout type and column count**: Set the layout type [`list-type`](#required-list-type) to `flow` (grid layout) or `waterfall` (waterfall layout), and set [`span-count`](#required-span-count) >= 2.
...
## List-specific CSS Properties

:::note

List-specific CSS properties are a set of CSS properties designed exclusively for styling the `<list>` component. These properties, such as `list-main-axis-gap` and `list-cross-axis-gap`, provide fine-grained control over the layout and spacing of items within the list. It is important to note that these properties will only work on `<list>` and will be ignored on any other component.
...
Controls the layout type of the `<list>` component, which needs to be used in conjunction with [`span-count`](#required-span-count).
...
When the `engineVersion` version is less than <VersionBadge v={3.4} />, this attribute will not take effect, and the [`scroll`](#scroll) event will not be intercepted. When the `engineVersion` version is greater than or equal to <VersionBadge v={3.4} />, if this attribute is not set, it will use default value 200 ms.
...
This property controls whether the list runs animations while the data source is being updated. It currently supports insert, delete, and update animations.

Example of update-animation:
...
>
  <tr style={{ height: '300px' }}>
    <td style={{ padding: '5px' }}>
      <center>
        <img
          src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/plugin/static/5_delete.gif"
    ...
    <td style={{ padding: '5px' }}>
      <center>
        <img
          src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/plugin/static/5_update.gif"
          style={{ width: '100%', height: '100%', objectFit: 'contain' }}
        />
        <br />
        <span>update</span>
    ...
    <td style={{ padding: '5px' }}>
      <center>
        <img
          src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/plugin/static/5_insert.gif"
          style={{ width: '100%', height: '100%', objectFit: 'contain' }}
        />
        <br />
        <span>insert</span>
...
Currently, `update-animation` can only work when `list-type` is `single` or `flow`.
...
Enables the scrollbar on Clay when the list content overflows. Clay uses the same [`scroll-bar-auto-hide`](/api/elements/built-in/scroll-view#scroll-bar-auto-hide), delay, sizing, and color attributes as `<scroll-view>`. Auto-hide defaults to `true` with a `1000` ms delay; set `scroll-bar-auto-hide={false}` to keep the scrollbar visible while content overflows.

### `scroll-bar-enable`

<IOSOnly /> <HarmonyOnly /> <ClayOnly />
...
Enables the scrollbar when the list content overflows. On Clay, this is a deprecated alias for [`enable-scrollbar`](#enable-scrollbar) and defaults to `false`. Harmony also defaults to `false`. On iOS, the default depends on the active list implementation, so set this attribute explicitly when scrollbar visibility matters.
...
```tsx
// DefaultValue: false
'scrollbar-enable'?: boolean
```

Enables the scrollbar for Web Lynx `<list>`. This Web-only spelling is not an alias for Clay's `enable-scrollbar` on other platforms.
...
On Harmony, when the contentSize is smaller than its own container size, set whether to enable the scroll effect.
...
When there are multiple sticky nodes, after a sticky node is pushed out of the `<list>` area by other sticky nodes, the node is recycled to address the memory overhead issue caused by too many sticky nodes. This optimization is enabled by default when `engineVersion` version is greater than or equal to <VersionBadge v={3.4} />.
...
| itemKey   | string  | null    | NO       | Specifies the `item-key` of the node to scroll to. If set `position` and `item-key`, the list will scroll to the node with the specified `item-key`.                                                                                                                                                                                                                                                                                                        |
...
Ensure that the [`sticky`](#sticky) attribute of the `<list>` component is set to `true` to allow child nodes to be sticky. You can also set the [`sticky-offset`](#sticky-offset) to determine the sticky position.
...
<Go
  img="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/lynx-website/assets/list_horizontal_snap.gif"
  example="list"
...
<APITable query="elements/list-item" />
