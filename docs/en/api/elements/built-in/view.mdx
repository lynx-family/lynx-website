...
The inner array is an array containing `4` strings, namely `x`, `y`, `width`, `height`, indicating the position of the touch area in the target node.
...
Specify the active regions for `event-through`.

The inner array contains an array of `4` strings: `x`, `y`, `width`, and `height`, representing the location of the touch region within the target node.
...
This event can be observed and analyzed by registering a [`PerformanceObserver()`](/api/lynx-api/performance-api/performance-observer/observe).For more detailed usage, see the [Marking Lynx Pipeline](/guide/performance/metrics/timing-flag).
