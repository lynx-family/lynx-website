...
### `pan-intercept-direction` <AndroidOnly /> <IOSOnly />
...
### `pan-intercept-scope` <AndroidOnly /> <IOSOnly />
...
:::note
The following three iOS filtering properties require `@lynx-js/types` 4.1.0 or later. Their iOS SDK runtime support begins with 4.1.
:::

### `ios-pan-intercept-gesture-class` <IOSOnly />
...
Specifies the class name of another platform-layer `UIGestureRecognizer` to intercept through `pan-intercept`. Use it together with [`ios-pan-intercept-view-class`](#ios-pan-intercept-view-class), [`ios-pan-intercept-view-tag`](#ios-pan-intercept-view-tag), [`pan-intercept-direction`](#pan-intercept-direction), and [`pan-intercept-scope`](#pan-intercept-scope).

### `ios-pan-intercept-view-class` <IOSOnly />
...
Specifies the class name of the `UIView` to which the other platform-layer gesture is attached. Use it together with [`ios-pan-intercept-gesture-class`](#ios-pan-intercept-gesture-class), [`ios-pan-intercept-view-tag`](#ios-pan-intercept-view-tag), [`pan-intercept-direction`](#pan-intercept-direction), and [`pan-intercept-scope`](#pan-intercept-scope).

### `ios-pan-intercept-view-tag` <IOSOnly />
...
Specifies the `UIView.tag` of the view to which the other platform-layer gesture is attached. Use it together with [`ios-pan-intercept-gesture-class`](#ios-pan-intercept-gesture-class), [`ios-pan-intercept-view-class`](#ios-pan-intercept-view-class), [`pan-intercept-direction`](#pan-intercept-direction), and [`pan-intercept-scope`](#pan-intercept-scope).

This property participates in matching only when explicitly set. If it is omitted, a target view whose `tag` is `0` does not match this filter.

When the swipe direction matches `pan-intercept-direction` and the view attached to the other platform-layer gesture is within the range specified by `pan-intercept-scope`, the other platform-layer gesture is intercepted only if all of the following conditions are met:

- The gesture recognizer is an instance of the class specified by `ios-pan-intercept-gesture-class` or one of its subclasses.
- The view attached to the gesture is an instance of the class specified by `ios-pan-intercept-view-class` or one of its subclasses.
- The `tag` of the view attached to the gesture equals `ios-pan-intercept-view-tag`.

All three iOS properties must be explicitly set, and the class names must be resolvable by the Objective-C runtime. For example:

```tsx
<view
  pan-intercept-direction={0}
  pan-intercept-scope={5}
  ios-pan-intercept-gesture-class="UIPanGestureRecognizer"
  ios-pan-intercept-view-class="UIView"
  ios-pan-intercept-view-tag={1001}
/>
```
...
For non-`<page>` nodes, this property supports inheritance. If the hit-tested node does not set `event-through`, it inherits the `event-through` value from its ancestors, excluding the `event-through` value of the `<page>` node.

For the `<page>` node, event pass-through is determined jointly by its `event-through` property and the page configuration [`Config.enableEventThrough`](../../rspeedy/type-config.config.enableeventthrough.md). When a touch hits the `<page>` node's area, Lynx does not consume the platform-level touch event if either value is `true`. This configuration property is available in `@lynx-js/type-config` 4.1.0 or later; this package floor is independent of its LynxSDK 3.2 runtime floor.

The `event-through` result is based on the node hit by the touch. [`user-interaction-enabled`](#user-interaction-enabled) and [`pointer-events`](../../css/properties/pointer-events.mdx) affect hit testing and the resulting hit node, so they can indirectly affect whether `event-through` takes effect. After the hit node is determined, Lynx applies the `<page>` rules above if the hit node is `<page>`; otherwise, it evaluates the `event-through` values of the hit node and its non-`<page>` ancestors.

The active region of this property is affected by [`event-through-active-regions`](#event-through-active-regions). If not set, it defaults to the entire region of the target node.

:::tip
`event-through` only controls whether Lynx responds to or consumes touch events from the platform layer. Passing unconsumed touch events to other platform-layer nodes also requires client-side integration for each platform:

- **Android**: Ensure that the parent container of the Lynx page does not consume touch events. When Lynx leaves an event unconsumed, the system's event dispatch can pass it to a sibling node that consumes it, such as a scroll container.
- **iOS**: When LynxView does not respond to a touch, its `hitTest` returns `nil` and the Lynx page is excluded from the event responder chain. The following example assumes that a dedicated Lynx wrapper contains LynxView and overrides `hitTest:withEvent:`. The intended responder is a sibling of that wrapper under their common ancestor:

  ```text
  Common ancestor
  ├── Lynx wrapper (overrides hitTest:withEvent:)
  │   └── LynxView
  └── Intended responder view
  ```

  When LynxView does not respond, the wrapper also returns `nil`, allowing the common ancestor to continue hit testing the wrapper's sibling. If the intended responder is instead a sibling of LynxView inside the wrapper, do not use this override unchanged: it replaces a non-Lynx descendant returned by `[super hitTest:withEvent:]` with `nil`.

  ```objc
  - (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    UIView *result = [super hitTest:point withEvent:event];
    if (result == nil || [result isDescendantOfView:_lynxView]) {
      return result;
    }
    return nil;
  }
...
- **HarmonyOS**: Under the nearest common ancestor of the Lynx page and the target responder node, locate the direct child that contains the Lynx page. In that node's `onTouchIntercept`, call the container's `canConsumeTouchEvent(x, y)`. Return `HitTestMode.Default` when Lynx should consume the event, or `HitTestMode.Transparent` to pass the event to a sibling node.

  ```ts
  .onTouchIntercept((event: TouchEvent) => {
    if (event && event.touches) {
      const touch = event.touches[0]
      const lynxConsumes = lynxComponent.canConsumeTouchEvent(
        touch.displayX,
        touch.displayY,
      )
      return lynxConsumes ? HitTestMode.Default : HitTestMode.Transparent
    }
