...
For the `<page>` node, event pass-through is determined jointly by its `event-through` property and the page configuration [`Config.enableEventThrough`](https://lynxjs.org/api/rspeedy/type-config.config.enableeventthrough). When a touch hits the `<page>` node's area, Lynx does not consume the platform-level touch event if either value is `true`. This configuration property is available in `@lynx-js/type-config` 4.1.0 or later; this package floor is independent of its LynxSDK 3.2 runtime floor.
...
- **iOS**: When LynxView does not respond to a touch, its `hitTest` returns `nil` and the Lynx page is excluded from the event responder chain. To let a sibling node respond to the event, the parent container's `hitTest` must also return `nil` as appropriate.
