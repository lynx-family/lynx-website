...
Lynxtron extends the renderer's capabilities with custom native modules via UI/texture extension C-APIs and [Node-API](/lynxtron/Native-Libraries/NodeJS-Native-Modules). The UI/texture extension C-APIs demonstrated here are experimental and are not yet part of the stable API reference; see [Lynx Native Libraries](/lynxtron/Native-Libraries/Lynx-Native-Libraries) for the documented integration boundary. In desktop development, it is common for part of an application to need native-rendered graphics or native UI: a camera preview, video surface, map view, drawing canvas, platform control, or any surface that needs hardware access, deeper GPU integration, or system-provided rendering. Lynxtron can combine those native surfaces or native UI with the Lynx runtime, so native-rendered parts stay native while interaction and UI logic can still be built and iterated with Lynx UI.
