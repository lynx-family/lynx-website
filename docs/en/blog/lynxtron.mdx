...
Lynxtron extends the renderer's capabilities with custom native modules via the public [extension-module C API](https://github.com/lynx-family/lynx/blob/develop/platform/embedder/public/capi/lynx_extension_module_capi.h), [native-view and texture-surface C API](https://github.com/lynx-family/lynx/blob/develop/platform/embedder/public/capi/lynx_native_view_capi.h), and [Node-API](/lynxtron/Native-Libraries/NodeJS-Native-Modules). See [Lynx Native Libraries](/lynxtron/Native-Libraries/Lynx-Native-Libraries) for the package and registration model. In desktop development, it is common for part of an application to need native-rendered graphics or native UI: a camera preview, video surface, map view, drawing canvas, platform control, or any surface that needs hardware access, deeper GPU integration, or system-provided rendering. Lynxtron can combine those native surfaces or native UI with the Lynx runtime, so native-rendered parts stay native while interaction and UI logic can still be built and iterated with Lynx UI.
