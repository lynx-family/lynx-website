...
> A `LynxWindow` hosts the Lynx bundle directly, and the example measures the incremental memory cost of opening a second window in the same process.
...
> The same Lynx UI consumes one notes API while the adapter selects a desktop bridge or Web bridge at runtime.
