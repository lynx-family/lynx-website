...
# Lynx 3.5: Main Thread Script, Fetch API, HarmonyOS Capability Alignment
...
- The Fetch API is now fully compatible with HarmonyOS and introduces support for chunked transfer encoding.
...
With Lynx 3.5, the rendering process has been improved so that the data structure for event handling is built synchronously during the first frame render. This means users can interact with the page as soon as it appears, resulting in a more responsive experience.
...
### Deferred Creation of `<list-item>` VNode

When working with long lists, ReactLynx used to create VNodes for every item all at once. This could slow down the initial rendering.
