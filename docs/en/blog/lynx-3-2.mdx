---
title: "What's new in Lynx 3.2"
date: 2025-05-14
sidebar: false
authors: ['shiwentao', 'lynx']
description: 'Lynx 3.2 delivers updates across its framework, engine, and tools, including the ReactLynx testing tools, `llms.txt` support, new Grid Layout features, improvements to `<list>` and `<text>`, and more.'
---
...
<BlogHeader authors={['shiwentao', 'lynx']} />
...
Lynx 3.2 delivers updates across its framework, engine, and tools, including the [ReactLynx testing tools](#reactlynx-testing-library), [`llms.txt` support](#website-llmstxt-support), [new Grid Layout features](#css-grid-layout-minmax-max-content-fit-content), improvements to [`<list>`](#improvements-to-list) and [`<text>`](#text-can-be-customized-to-select-across-multiple-elements), and more, to bring you more familiar development experience and more capabilities. With over [296 commits](https://github.com/lynx-family/lynx/releases/) from 61 contributors (including some first-time contributors). Let’s explore what’s new!

## ReactLynx Testing Library

[Testing Library](https://testing-library.com/) is a popular way in the JavaScript community to test UI components, and we've adapted it for Lynx. We now introduce a new package [`@lynx-js/react/testing-library/`](/api/react/testing-library) to provide testing abstractions such as [`render`](/api/react/testing-library#render). It can also be used with the official [`@testing-library/jest-dom`](https://github.com/testing-library/jest-dom) to assert the presence and behavior of elements using matchers such as [`toBeInTheDocument`](https://github.com/testing-library/jest-dom?tab=readme-ov-file#tobeinthedocument).
...
[Learn more about implementing Pagination with `item-snap`.](/api/elements/built-in/list.html#pagination-with-item-snap)
...
Rspeedy [v0.9.0](https://github.com/lynx-family/lynx-stack/discussions/482) is now available, featuring significant improvements in size and performance. In v0.9.0, Rspeedy bundles with [Rslib](https://rslib.rs/), reducing installation size by 50%. The upgrade to [Rspack](https://rspack.rs/) [1.3](https://rspack.rs/blog/announcing-1-3) delivers enhanced code splitting, optimized bundle size, and improved memory efficiency. Additional enhancements include new CLI flags and configurations. For more details, refer to the complete full CHANGELOG [`@lynx-js/rspeedy v0.9.0`](https://github.com/lynx-family/lynx-stack/blob/main/packages/build/core/CHANGELOG.md#090).
...
Download the latest DevTool Desktop Application from the [DevTool Releases](https://github.com/lynx-family/lynx-devtool/releases) page to get started.
