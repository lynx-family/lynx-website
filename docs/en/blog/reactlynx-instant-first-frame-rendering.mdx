...
With responsibilities split across two threads, a single React entry module can no longer emit just one piece of code. ReactLynx's build toolchain, [Rspeedy](/guide/build/lynx-bundle), compiles each entry into dual-threaded artifacts: `main-thread.js` on the main thread, which creates the Elements of the first frame along the first-screen path; and `background.js` on the background thread, preserving the complete React logic. In default development builds, these two files can typically be found under [`dist/.lynx/[name]/`](/guide/build/output). Ultimately, they are packaged together into the same `lynx.bundle` and loaded per thread by the Lynx engine.
...
Using the earlier `App` component that contains side effects, event handlers, dynamic text, and [`__MAIN_THREAD__`](/api/react/Documents/Built-in_macros) branches, the main-thread artifact after ReactLynx compilation looks roughly like this:
