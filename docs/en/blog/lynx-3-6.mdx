...
In Lynx 3.6, CSS variables can be defined and referenced inline. This allows CSS variables to be applied at component boundaries directly, without relying on ancestor class switches (such as `theme-light` / `theme-dark`) or imperative JS `setProperty` calls.

It becomes essential when a component's style is user-selected or computed at runtime, since such values cannot be represented by a finite set of predefined classes and would otherwise require imperative style updates.
