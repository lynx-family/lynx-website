...
### Inline CSS Variables for Scoped Theming

Before Lynx 3.6, theme switching typically happened from the outside of a component: either updating CSS variables via JS `setProperty`, or switching an ancestor class (such as `theme-light` / `theme-dark`) to change variable values indirectly.

In Lynx 3.6, CSS variables can be defined and overridden inline. This makes theming a first-class, component-level capability:: a component can accept theme overrides through its props and apply them directly at its own boundary, without requiring a global wrapper or imperative JS updates.
...
### Nested CSS Variables for Theme Layering
