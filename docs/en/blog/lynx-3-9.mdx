...
On the build side, a dual-thread Minimizer mechanism performs finer-grained compression and tree-shaking across main-thread and background-thread bundles. You can enable the default optimization through [`optimizeBundleSize`](/api/packages/react-rsbuild-plugin#optimizebundlesize), or configure the two separately with [`Minify.mainThreadOptions`](https://rsbuild.rs/config/output/minify) and [`Minify.backgroundOptions`](https://rsbuild.rs/config/output/minify).
