...
At compile time ReactLynx extracts the static element structure of your JSX. The current compiler records it as a compiled representation called a **Snapshot**, so at runtime only its dynamic parts need updating. Learn more in [Compiler-Informed Rendering](../react/snapshot.mdx).
