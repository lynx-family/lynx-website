...
AnimaX (/ˈænɪmæks/) is an open-source C++ motion framework from the Lynx family, built on the Skity rendering engine. Its source code is available in [lynx-family/animax](https://github.com/lynx-family/animax). It extends Lottie capabilities with support for advanced AE features like particle animations, 3D transformations, and complex gradients.

### Alpha Video

A video format where transparency information is stored alongside RGB data, typically by tiling the original video (left side: alpha channel, right side: RGB). Used to render AE effects that Lottie cannot natively support.
...
- Alpha Video preview not yet supported
