...
[`pluginLynx`](https://www.npmjs.com/package/@lynx-js/rsbuild-plugin) (enabled by
...
Wire it into `rsbuild.config.ts`. You upload the file as-is; the `release` your
...
tapping `LynxTemplatePlugin.beforeEncode`, obtained via the exposure that
framework plugins such as `pluginReactLynx` publish:
