...
**Returns:** [`InitData`](/react/api/types#initdata)
...
| --- | --- | --- |
| `callback` | `(data: `[`InitData`](/react/api/types#initdata)`) => void` |  |
...
**Returns:** [`GlobalProps`](/react/api/types#globalprops)
...
| Parameters | Type | Description |
| --- | --- | --- |
| `callback` | `(data: `[`GlobalProps`](/react/api/types#globalprops)`) => void` |  |
...
**Returns:** [`MainThreadRef`](/react/api/types#mainthreadref)`<T>`
...
| Parameters | Type | Description |
| --- | --- | --- |
| `effect` | `EffectCallback` (from @types/react) | Imperative function that can return a cleanup function |
| `deps?` | `DependencyList` (from @types/react) | If present, effect will only activate if the values in the list change (using ===). |
...
| --- | --- | --- |
| `effect` | `EffectCallback` (from @types/react) | Imperative function that can return a cleanup function |
| `deps?` | `DependencyList` (from @types/react) | If present, effect will only activate if the values in the list change (using ===).  `useLayoutEffect` in the background thread cannot offer the precise timing for reading layout information and synchronously re-render, which is different from React. |
