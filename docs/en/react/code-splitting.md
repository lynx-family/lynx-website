...
By default the loader follows [`engineVersion`](/plugins/plugin-react#engineversion): `FetchBundle` when it is `3.9` or higher, `QueryComponent` otherwise. `engineVersion` defaults to `3.2`, so a build that does not raise it to `3.9` gets `QueryComponent`.
...
[`experimental_isLazyBundle`]: /plugins/plugin-react#experimental-islazybundle
