...
ReactLynx itself is an "idiomatic" React, but it also pioneered optimizations such as "dual-threaded React" and compiler-informed rendering.
...
- **[Dual-threaded in Mind](./thinking-in-reactlynx.mdx)**: ReactLynx follows the programming model of React, but by utilizing the **dual-threaded runtime** provided by Lynx, combined with its own programming paradigm (or rules) to achieve better performance and user experience.
  ...
  - **[Lifecycle under dual-thread architecture](./lifecycle.mdx)**: Due to the dual-thread architecture of Lynx, the lifecycle of ReactLynx is slightly different from that of traditional React in terms of calling timing.
- **[Compiler-Informed Rendering](./compiler-informed-rendering.mdx)**: ReactLynx extracts static JSX structures into a reusable compiled representation so the runtime can focus on dynamic values and subtrees.
...
For a full list of APIs ReactLynx implemented, please refer to the [`@lynx-js/react` API documentation](/react/api/).
...
For commonly used Web methods such as `e.stopPropagation()` and `e.preventDefault()`, Lynx only supports [`e.stopPropagation()`](/api/lynx-api/event/event.html#stoppropagation) in [Main Thread Scripts](./main-thread-script.html). But Lynx's [Event Propagation Mechanism](/guide/interaction/event-handling/event-propagation.html) allows you to implement similar functions or effects as on the Web.
...
Continue with [Compiler-Informed Rendering](/react/compiler-informed-rendering) and [Main Thread Scripts](/react/main-thread-script) to learn more about rendering and main-thread interactions.

### Build ReactLynx Applications
...
<NextSteps.Root>
  <NextSteps.Step
    ...
    title="Guides"
    description="Explore best practices, code splitting, React Compiler, and code testing"
  />
  <NextSteps.Step
    href="/react/data-fetching"
    title="Integrations"
    description="Use ecosystem solutions for data fetching, state management, routing, and Generative UI"
  />
</NextSteps.Root>

**Extra Topics**: Consult the [API Reference](/react/api/) and use [Framework Performance Profiling](/guide/performance/analysis-performance/analysis-framework) to investigate performance issues.
