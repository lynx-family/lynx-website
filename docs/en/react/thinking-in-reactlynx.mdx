...
2. The **background thread** runs with a complete React runtime, handling component lifecycles and other side effects. Since complete consistency with single-threaded React is not possible, we have a modified component lifecycle as shown in [Component Lifecycle](./lifecycle.mdx).
...
[`'background only'`]: /api/react/Documents/Directives
