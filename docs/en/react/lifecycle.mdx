...
To solve the slow first-screen loading problem of traditional web applications, Lynx adopts an innovative rendering strategy: when the application starts, the main thread completes the first-screen rendering to ensure the fastest possible initial display. Meanwhile, the background thread performs a parallel rendering and builds a node tree structure, then compares the tree structures from both threads and synchronizes their differences to prepare for subsequent updates.
...
[`main-thread:ref`]: /react/main-thread-script.html#using-main-threadref-to-obtain-node-objects
