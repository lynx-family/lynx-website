...
The compiler generates reusable templates, while the runtime executes their creation and update logic. In ReactLynx's dual-thread architecture, the same template participates in both [Instant First-Frame Rendering (IFR)](/guide/interaction/ifr) and subsequent updates.

The same template can be reused across multiple mount locations, but each mount has its own pair of template instances: the main-thread instance owns the Lynx elements it creates, while the background-thread instance stores the state and dynamic data used for reconciliation. The initial synchronization establishes the correspondence between these instances for subsequent updates.

### Initial Render: Create and Associate Template Instances

During the initial render, the main thread invokes the template's `create()` and `update()` methods to directly create the static host structure and apply the initial dynamic properties, then mounts dynamic children into the reserved slots. The background thread also creates the corresponding instance for this render. The following pseudocode illustrates the responsibilities and correspondence of the two sides, not their actual execution order:

```js
function renderFirstScreen(Template, dynamicData) {
  const mainInstance = Template.create();
  Template.update(mainInstance, dynamicData);
  mountDynamicChildren(Template, mainInstance, dynamicData);

  const backgroundInstance = renderOnBackgroundThread(Template, dynamicData);

  connectInstances(backgroundInstance, mainInstance);
}
```

Here, `Template` represents any compiler-generated template whose `create()` and `update()` methods have the conceptual shape shown in the previous section. `connectInstances()` summarizes the initial synchronization: the two sides do not share one instance, but establish a correspondence so that subsequent changes from the background thread can be applied to the main-thread instance created for the first screen.

### Updates: Send Only Dynamic Changes

After this correspondence is established, the background thread compares the dynamic data: changes to dynamic properties can be handled by targeted updaters generated by the compiler, while dynamic children remain reconciled by React.
