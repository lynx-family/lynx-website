...
The initial background-thread reconciliation also creates a background template-instance tree that corresponds to the main-thread template-instance tree. During [First-frame Rendering](/guide/spec.html#first-frame-rendering), hydration establishes the mapping between template instances on the two threads, allowing subsequent updates to locate the main-thread template instance created for the first frame by `instanceId`.
