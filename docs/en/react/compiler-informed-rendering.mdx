...
When a page loads, the main thread renders the first-screen content directly to achieve [Instant First-Frame Rendering (IFR)](/guide/interaction/ifr). The Lynx specification calls this [First-frame Rendering](/guide/spec.html#first-frame-rendering), a special type of [Main Thread Rendering](/guide/spec.html#main-thread-rendering). Meanwhile, the background thread performs a parallel render; the initial synchronization compares the two template-instance trees and synchronizes their differences in preparation for subsequent updates.
