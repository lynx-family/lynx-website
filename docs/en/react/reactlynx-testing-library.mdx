...
The [`@lynx-js/react/testing-library`](/react/api/testing-library) package provides the same set of APIs (e.g. [`render`](/react/api/testing-library#render), [`fireEvent`](/react/api/testing-library#fireevent), [`screen`](/react/api/testing-library#screen), etc.) for testing the rendering result of ReactLynx, just like the popular [React Testing Library](https://testing-library.com/docs/react-testing-library/intro/), with the underlying dual-threaded architecture abstracted away by the [@lynx-js/testing-environment](/packages/testing-environment).
...
The [`render`](/react/api/testing-library#render) method is used to render a ReactLynx component and returns a [`RenderResult`](/react/api/testing-library#renderresult) object. The `container` field is a [`LynxElement`](/packages/testing-environment#lynxelement) containing the rendering result.
...
In the `render` process, the event handler will be mounted to the [`eventMap`](/packages/testing-environment#lynxelement) property of the [`LynxElement`](/packages/testing-environment#lynxelement) object, so you can use the `eventMap` property to get the event handler of the element for assertion.
...
You can use [`screen`](/react/api/testing-library#screen) to query page elements, it provides some common methods, such as [`getByText`](/react/api/testing-library#getbytext), [`getByTestId`](/react/api/testing-library#getbytestid), etc. There are also methods like [`waitForElementToBeRemoved`](/react/api/testing-library#waitforelementtoberemoved) to wait for the state of page elements.
...
In the second parameter [`RenderOptions`](/react/api/testing-library#renderoptions) of the [`render`](/react/api/testing-library#render) method, there are two options [`enableMainThread`](/react/api/testing-library#renderoptions) and [`enableBackgroundThread`](/react/api/testing-library#renderoptions). `enableMainThread` is used to enable [IFR](../guide/interaction/ifr.mdx) and `enableBackgroundThread` is used to enable background thread rendering.
...
See details in [API Reference](/react/api/testing-library).
