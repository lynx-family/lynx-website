---
description: 'Test ReactLynx components using a Testing Library-style API.'
---

# Code Testing

The [`@lynx-js/react/testing-library`](/api/react/testing-library) package provides the same set of APIs (e.g. [`render`](/api/react/testing-library#render), [`fireEvent`](/api/react/testing-library#fireevent), [`screen`](/api/react/testing-library#screen), etc.) for testing the rendering result of ReactLynx, just like the popular [React Testing Library](https://testing-library.com/docs/react-testing-library/intro/), with the underlying dual-threaded architecture abstracted away by the [@lynx-js/testing-environment](/api/packages/testing-environment).

## Setup

### From `@lynx-js/create-lynx`

A project created with [`@lynx-js/create-lynx`](https://www.npmjs.com/package/@lynx-js/create-lynx) comes with Rstest and ReactLynx Testing Library already set up. Run the tests with `npm run test`.

### Adding to an existing project

ReactLynx Testing Library is integrated in the `testing-library` subdirectory of the `@lynx-js/react` package and can be used directly. Currently we support the testing frameworks Rstest and Vitest.

#### Rstest

Setup [Rstest](https://rstest.rs/) with `@lynx-js/react/testing-library/rstest-config`.
...
:::info
The following code examples are all implemented in Vitest, but you can also use them in Rstest. Replace `vitest` with `@rstest/core` and `vi` with `rstest` in the code examples to use them under Rstest.
:::
...
The [`render`](/api/react/testing-library#render) method is used to render a ReactLynx component and returns a [`RenderResult`](/api/react/testing-library#renderresult) object. The `container` field is a [`LynxElement`](/api/packages/testing-environment#lynxelement) containing the rendering result.
...
In the `render` process, the event handler will be mounted to the [`eventMap`](/api/packages/testing-environment#lynxelement) property of the [`LynxElement`](/api/packages/testing-environment#lynxelement) object, so you can use the `eventMap` property to get the event handler of the element for assertion.
...
You can use [`screen`](/api/react/testing-library#screen) to query page elements, it provides some common methods, such as [`getByText`](/api/react/testing-library#getbytext), [`getByTestId`](/api/react/testing-library#getbytestid), etc. There are also methods like [`waitForElementToBeRemoved`](/api/react/testing-library#waitforelementtoberemoved) to wait for the state of page elements.
...
In the second parameter [`RenderOptions`](/api/react/testing-library#renderoptions) of the [`render`](/api/react/testing-library#render) method, there are two options [`enableMainThread`](/api/react/testing-library#renderoptions) and [`enableBackgroundThread`](/api/react/testing-library#renderoptions). `enableMainThread` is used to enable [IFR](../guide/interaction/ifr.mdx) and `enableBackgroundThread` is used to enable background thread rendering.
...
See details in [API Reference](/api/react/testing-library).
