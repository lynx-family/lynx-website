...
When configuring [Rstest](https://rstest.rs/), you can directly reuse the configuration in `lynx.config.ts` and specify the built-in `setupFiles` of the ReactLynx Testing Library.
...
To configure [Vitest](https://vitest.dev/), you need to use the `vitestTestingLibraryPlugin` plugin in `@lynx-js/react/testing-library/plugins`.
