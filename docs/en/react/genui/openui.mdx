...
The example below streams a mock OpenUI Lang response into the Renderer. As statements arrive, forward references resolve and the plan picker appears progressively. Choose a plan or billing cycle to update local `$variables`, select **Continue** to see the host-facing Action, or select **Replay** to restart the stream.

<Go
  example="openui"
  defaultFile="src/plan-picker/PlanPicker.tsx"
  defaultEntryFile="dist/plan-picker.lynx.bundle"
  entry="src/plan-picker"
  defaultTab="web"
/>
