...
[`@lynx-js/genui/openui`](/api/genui/openui) is the OpenUI Lang renderer for [ReactLynx](/react/introduction). It incrementally parses model output, maps recognized expressions to registered ReactLynx components, and renders the result through Lynx's native rendering pipeline.
...
For individual symbols and props, see the [`@lynx-js/genui/openui` API reference](/api/genui/openui). The implementation is developed in the [Lynx Stack OpenUI source](https://github.com/lynx-family/lynx-stack/tree/main/packages/genui/openui).
