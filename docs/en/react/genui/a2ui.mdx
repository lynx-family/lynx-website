...
which the Renderer validates and renders against a pre-authorized Catalog, mapping the protocol messages into a native UI tree.
...
[@lynx-js/genui/a2ui](https://www.npmjs.com/package/@lynx-js/genui) is the A2UI rendering implementation for ReactLynx.
...
The key point is that A2UI does not let the model generate UI code freely. It lets the model compose UI dynamically within an authorized scope, while the business still controls components, visual style, callable capabilities, and safety boundaries.
...
- The Renderer first exposes Catalog entries such as weather cards and city buttons, and establishes a connection with the Agent.
