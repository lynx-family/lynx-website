...
:::info DevTool MCP vs DevTool Skill
There are two ways for coding agents to interact with Lynx DevTool:

- **DevTool MCP** (this page) — MCP server protocol, works with MCP-compatible clients.
- **[DevTool Skill](/ai/skills#devtool)** — CLI-based, works with any coding agent (Claude Code, Cursor, Copilot), installed via `npx skills add`.
...
<a href="https://cursor.com/en/install-mcp?name=lynx-devtool&config=eyJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsIkBseW54LWpzL2RldnRvb2wtbWNwLXNlcnZlckBsYXRlc3QiXX0=">
  <img
    ...
    alt="Install Lynx DevTool MCP in Cursor"
