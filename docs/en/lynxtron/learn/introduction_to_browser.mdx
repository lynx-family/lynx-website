...
import browserDemoGif from '@assets/lynxtron/browser-demo.gif?url';
import browserDemoFrameLessPng from '@assets/lynxtron/browser_demo_frame_less.png?url';
import browserTopBarPng from '@assets/lynxtron/browser_top_bar.png?url';
import browserDemoTopBarDragGif from '@assets/lynxtron/browser_demo_top_bar_drag.gif?url';
import browserDemoControlGif from '@assets/lynxtron/browser_demo_control.gif?url';
import browserDemoNewTabGif from '@assets/lynxtron/browser_demo_new_tab.gif?url';
import browserDemoWebviewPng from '@assets/lynxtron/browser_demo_webview.png?url';
import browserDemoSwitchTabGif from '@assets/lynxtron/browser_demo_switch_tab.gif?url';
import browserDemoGoBackGif from '@assets/lynxtron/browser_demo_go_back.gif?url';
...
### Set Up the Native `<webview>` Element

Lynxtron provides `<webview>` through the optional `@lynx-js/cef-webview`
native library. Install it before following the browser-specific steps:

<PackageManagerTabs command="install @lynx-js/cef-webview" />

Projects created with `create-lynxtron` already configure `pluginLynxtron()` for
the desktop Rsbuild environment. The plugin AutoLinks installed native
libraries, so do not add a second instance of it.

Import and initialize CEF in the desktop main process before creating any
`LynxWindow`:
...
#### macOS: Sync the CEF Frameworks

On macOS, the CEF Framework and Helper apps must also be present in the
installed Lynxtron app bundle. Add the following project-level helper, based on
the [official browser demo](https://github.com/lynx-family/lynxtron/blob/main/src/packages/browser-demo/scripts/postinstall.js),
as `scripts/sync-cef-frameworks.cjs`:
...
async function main() {
  ...
  const cefRoot = packageRoot('@lynx-js/cef-webview');
  const lynxtronRoot = packageRoot('@lynx-js/lynxtron');
  ...
  const appBundle = [
    path.join(lynxtronRoot, 'dist', 'Lynxtron.app'),
    path.join(lynxtronRoot, 'dist', 'lynxtron.app'),
    path.join(lynxtronRoot, 'dist', 'darwin', process.arch, 'lynxtron.app'),
  ].find((candidate) => fs.existsSync(candidate));

  if (!fs.existsSync(source) || !appBundle) {
    throw new Error('CEF or Lynxtron macOS artifacts were not downloaded.');
  }

  const destination = path.join(appBundle, 'Contents', 'Frameworks');
...
Run it after dependencies are installed by adding it to `package.json`:

```json
{
  "scripts": {
    "postinstall": "node scripts/sync-cef-frameworks.cjs"
  }
}
```

The helper resolves the installed package locations instead of hard-coding a
`node_modules` layout. It uses the current `dist/Lynxtron.app` location while
retaining compatibility with the older platform-specific layout.

:::note pnpm 11 users

If pnpm 11 reports that it blocked dependency build scripts, add these entries
to `pnpm-workspace.yaml`, then run `pnpm install` again so the native artifacts
can be downloaded. This is not required for the other package-manager tabs or
when pnpm already permits the scripts.
...
<img src={browserDemoFrameLessPng} alt="Remove the title bar" />
...
<img src={browserTopBarPng} alt="Custom title bar" />
...
Lynxtron supports the CSS property [`-x-app-region: drag`](./Custom-Window-Styles#step-2-make-the-window-draggable-again) to define draggable regions, similar to Electron's `-webkit-app-region: drag`.
...
<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img src={browserDemoTopBarDragGif} width="100%" />
</div>
...
<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img src={browserDemoControlGif} width="100%" />
</div>
...
<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img src={browserDemoNewTabGif} width="100%" />
</div>

### Add the `<webview>` Component

Lynxtron provides the [`<webview>`](/api/elements/built-in/webview) element for loading external web pages. After importing it correctly, you can use the `<webview>` component to load web content.
...
<img src={browserDemoWebviewPng} alt="Load external web pages" />
...
<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img src={browserDemoSwitchTabGif} width="100%" />
</div>
...
<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img src={browserDemoGoBackGif} width="100%" />
</div>
