...
Before running `npm run dev`, close any other host application using this WebView, including an installed copy. The current CEF storage is shared across host applications; only one browser host process can initialize it at a time. Multiple tabs in this browser are supported. If `initialize()` throws, stop startup and inspect the error instead of continuing with a blank WebView.
...
const handleMinimize = () => {
  NativeModules.bridge.call('minimize', {}, (res: any) => {
    console.log(res);
  });
};
