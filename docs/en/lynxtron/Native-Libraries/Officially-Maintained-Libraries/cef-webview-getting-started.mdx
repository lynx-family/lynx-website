...
AutoLink loads the native library and makes its element registration available, but does not initialize CEF. The explicit import below gives the app access to `initialize()`. Call it after `app.whenReady()` and before creating a `LynxWindow` or loading a Lynx bundle:
