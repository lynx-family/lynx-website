...
The automatic loading and packaging flow on this page requires matching Lynxtron tooling with support for the `targets` manifest. Check [AutoLink tooling availability](/guide/autolink#set-up-autolink) before installing; this integration is still prerelease.
...
Only one host process can use the current shared CEF storage at a time. Stop other WebView host applications before running this example, including a previously launched dev or installed app. Multiple WebViews in one host are supported. `initialize()` throws on failure; do not ignore the error and continue creating windows. See [initialization limits](/lynxtron/api/@lynx-js/cef-webview/#initialization-result-and-process-limit).
