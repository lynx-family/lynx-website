...
CEF storage is isolated by application; multiple instances using the same storage directory may still conflict. `initialize()` throws on failure; do not ignore the error and continue creating WebViews. See [initialization details](/lynxtron/api/@lynx-js/cef-webview/#initialization-result-and-process-limit).
