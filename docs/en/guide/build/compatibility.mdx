...
If you try to run a Bundle on a lower version of the Lynx Engine, a Fatal level error with code [10204](/api/errors/error-code.mdx#10204-e_app_bundle_load_parse_failed) will occur, and the rendering process will stop. This mechanism prevents potential runtime issues caused by version incompatibility.
...
Lynx provides structured compatibility information for platform features, APIs, built-in components, and CSS properties.

For APIs, two paired components help you understand platform support:
...
The `<APITable />` component displays detailed, version-specific support information for each platform. It covers Android, iOS, HarmonyOS, and Web, including specific version numbers where applicable.
...
[`engineVersion`]: /api/build/react-rsbuild-plugin.pluginreactlynxoptions.targetsdkversion
