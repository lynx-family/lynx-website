import { CodeFold, Details, Go, PlatformTabs } from '@lynx';
import { Badge, PackageManagerTabs, Steps, Tab, Tabs } from '@theme';
...
To implement local persistent storage on different platforms, use the matching platform native API or the Lynx C/C++ APIs:
...
- Desktops(Node-API): Lynx C/C++ APIs and the weak-node-api C++ wrapper
...
1. **Create a Lynx project**: Refer to the [Create a Lynx Project](/guide/start/quick-start#create-a-new-lynx-project) guide to create your Lynx project.
...
Congratulations! You have successfully created a native module. If you want to create a native module in your application, you first need to integrate Lynx by referring to the [Integrate with Existing Apps](/guide/start/integrate-with-existing-apps) guide, and then follow the steps above to create the native module.
