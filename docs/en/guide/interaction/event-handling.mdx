...
When the event processing function is a [main thread script](../../react/main-thread-script.mdx), you need to add a [`'main thread'`](/api/react/Documents/Directives) directive to the first line of the function body to indicate that the function runs on the main thread.
