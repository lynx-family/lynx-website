...
To enable inheritance, configure[`enableCSSInheritance`](/plugins/plugin-react#enablecssinheritance).
...
In addition to the default inheritable properties, you can configure the page with [`customCSSInheritanceList`](/plugins/plugin-react#customcssinheritancelist)to define custom inheritable CSS properties. When there are custom inheritance declarations, only the CSS properties listed in the `customCSSInheritanceList` will be inheritable.
