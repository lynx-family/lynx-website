...
Put the behavior shared by all Desktops backends in the element header. Platform backends inherit from this class and only implement presentation details.
...
Each custom element is created as a `LynxNativeView`. The declaration, property handling, method handling, and static registration above are shared by all Desktops implementations. The backend decides what the view owns after creation: a platform control for Native UI, or a Lynx surface for Texture.

The examples below use the same registered creator name for two alternative backend implementations. Choose one backend in your library build so `CreateExplorerInputElementNativeView` resolves to one implementation.

<Tabs groupId="custom-component-desktop-backend">

<Tab label="Native UI">

The Native UI backend attaches a platform control and updates its frame in `OnLayoutChanged`. For `<explorer-input>`, the implementation attaches a minimal input control.
...
The shared texture backend is cross-platform. It enables the surface path, tracks layout size, and calls `DrawExplorerInputElementTextureSurface`, which each platform backend implements.
...
After the native code is ready, compile it into the host app or into a Desktops native library that the host can load. If your package contains both Native UI and Texture implementations, choose one in your build system so the registered creator resolves to one implementation.

Use the registered tag from Lynx after the library has been linked or loaded:
