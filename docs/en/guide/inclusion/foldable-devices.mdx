...
Updates merge the fields and emit [`onGlobalPropsChanged`](/react/api/global-events). With the default [`globalPropsMode: 'reactive'`](/plugins/plugin-react#globalpropsmode), **ReactLynx automatically re-renders the entire page**; components simply read the latest dimensions during render. The optional `'event'` mode disables this automatic render.
