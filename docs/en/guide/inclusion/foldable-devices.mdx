...
Updates merge the fields and emit [`onGlobalPropsChanged`](/api/react/Document.global-events#onglobalpropschanged). With the default [`globalPropsMode: 'reactive'`](/api/build/react-rsbuild-plugin.pluginreactlynxoptions.globalpropsmode), **ReactLynx automatically re-renders the entire page**; components simply read the latest dimensions during render. The optional `'event'` mode disables this automatic render.
