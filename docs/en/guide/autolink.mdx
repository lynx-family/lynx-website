      ...
      "targets": [
        {
          "os": "darwin",
          "arch": "arm64",
          "binaries": ["dist/darwin/arm64/lynx-button.node"],
          "frameworks": ["dist/darwin/arm64/frameworks"]
        },
        {
          "os": "win32",
          "arch": "x64",
          "binaries": ["dist/win32/x64/lynx-button.node"]
        }
      ]
    },
    "macos": {
...
For Lynxtron, use `targets` to group runtime payloads by host. Each target declares `os`, `arch`, and one or both string arrays named `binaries` and `frameworks`; every path is relative to the package root. Autolink selects the matching target, verifies its paths, stages the package, and loads its `./lynxtron` export. That entry reads the same manifest and loads the selected `.node`. `binaries` describes directly loadable native files such as `.node`, `.dylib`, and `.dll`; `frameworks` describes macOS Framework payload directories, which may also contain helper app bundles and must preserve their internal symbolic links.
...
- `lynx.lib.json` is the Autolink contract. It declares each platform-selected binary or Framework source artifact.
