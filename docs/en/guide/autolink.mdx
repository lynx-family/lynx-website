...
These paths identify source artifacts inside the npm package. The host application integration chooses their final location in the development output or packaged application. This Lynxtron manifest shape is still prerelease, so it does not provide legacy aliases: use the plural `binaries` field and the `arch` selector exactly as shown.
