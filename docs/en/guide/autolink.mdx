      ...
      "binaries": [
        {
          "os": "darwin",
          "arch": "arm64",
          "path": "dist/darwin/arm64/lynx-button.node"
        },
        {
          "os": "win32",
          "arch": "x64",
          "path": "dist/win32/x64/lynx-button.node"
        }
      ],
      "frameworks": [
        {
...
For Lynxtron, use `binaries` and `frameworks` to describe runtime payloads explicitly. Both fields are arrays, and every entry has an `os`, an `arch`, and a `path` (or an array of paths) relative to the package root. Autolink selects entries matching the target host, verifies them, stages the package, and loads its `./lynxtron` export. That entry reads the same manifest and loads the selected `.node`. `binaries` describes directly loadable native files such as `.node`, `.dylib`, and `.dll`; `frameworks` describes macOS Framework payload directories, which may also contain helper app bundles and must preserve their internal symbolic links.

These paths identify source artifacts inside the npm package. The host application integration chooses their final location in the development output or packaged application. This Lynxtron manifest shape is still prerelease, so it does not provide opaque package roots or legacy aliases: use `binaries`, `frameworks`, and the `arch` selector exactly as shown.
