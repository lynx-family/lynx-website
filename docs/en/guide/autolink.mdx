...
Use the Lynx native annotations and registration macros in library packages. Native Modules usually extend the spec generated by `lynx-autolink-codegen`; Elements and Services are discovered from their native markers.
...
Autolink uses Lynx native marker names as its public library authoring API.

For iOS packages, Autolink scans `@LynxNativeModuleRegister(...)`, `@LynxUIRegister(...)`, and `@LynxServiceRegister(...)` declarations so those packages can be linked through the generated registry.
