...
For Lynxtron, use `targets` to group runtime payloads by host. Each target declares `os`, `arch`, and one or both string arrays named `files` and `frameworks`; every path is relative to the package root. Autolink selects the matching target, verifies its paths, stages the package, and loads its `./lynxtron` export. That entry reads the same manifest and loads the selected `.node` from `files`. `files` lists every payload that remains at its package-relative location, including native addons, dynamic libraries, executables, resource packs, and directories. `frameworks` is reserved for macOS Framework payload directories, which may also contain helper app bundles and must preserve their internal symbolic links.

`npm pack` does not preserve symbolic-link entries inside package contents. If a Framework relies on links, provide an idempotent installation step that reconstructs them from shipped targets, then verify the installed package. Lynxtron preserves those restored links when it stages and packages the Framework.

These paths identify source artifacts inside the npm package. Lynxtron's development plugin and builder automatically choose their final locations in development output and packaged applications. App teams do not copy these artifacts or add per-library packaging rules. This Lynxtron manifest shape is still prerelease, so it does not provide opaque package roots or legacy aliases: use `targets`, `files`, `frameworks`, and the `arch` selector exactly as shown.
