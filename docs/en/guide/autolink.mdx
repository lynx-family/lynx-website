...
Create `hvigorconfig.ts` in the HarmonyOS project root and enable it once:
...
Before Hvigor creates module nodes, the plugin scans visible `node_modules` directories, generates a Registry HAR under the HAP module's `build/generated/lynx-autolink` directory, and adds the library and Registry HAR modules to the build graph through `hvigorConfig.includeNode()`. After the target HAP is evaluated, it uses HAP model APIs to inject the Registry dependency and generated resource directory and set `module.appStartup`. The plugin does not edit the app's `src` tree or checked-in `build-profile.json5` files.
