...
- For Lynxtron, the app needs a host-side Rspack build that can install npm packages and load native libraries through `pluginLynxtron()`.
...
Use `pluginLynxtron()` in the Lynxtron host-side Rspack config. The plugin includes Lynxtron Autolink by default: it scans installed dependencies for `lynx.lib.json`, stages matching native library packages, and injects code that requires each package's `./lynxtron` entry. That entry loads the current host's `.node` artifact.
...
For Lynxtron, `platforms.lynxtron.path` points to the package directory that Autolink stages with the library package, typically `dist`. `platforms.macos.sourceDir` and `platforms.windows.sourceDir` describe the shared C/C++ source directory for source-aware tooling. The package must export `./lynxtron` from `package.json`; that entry loads the current host's `.node` file, for example `dist/macos/arm64/lynx-button.node` or `dist/windows/x64/lynx-button.node`. Build and publish the native artifacts for each OS and architecture that the `./lynxtron` entry supports.
...
- `lynx.lib.json` is the Autolink contract. It tells the host app where Android and iOS source code lives, and where the Lynxtron native loading entry lives.
...
If the same package also ships Lynxtron native artifacts, it may contain shared native sources, a Lynxtron loading entry, and per-host outputs:
...
Lynxtron native libraries use the same Lynx static registration model. A package can expose native modules, elements, or both without adding a Lynxtron-specific registration API.

The package export should expose the Lynxtron loading entry:
...
A typical `lynxtron/index.cjs` loads the current host artifact:
...
- `types/index.d.ts` and `types/napi-native-module.d.ts` for typed Native Module declarations, when the library exposes Native Modules
- `src/index.ts` for the JavaScript facade
- `android/` and `ios/` native source folders, plus `shared/` native sources and a `lynxtron/` loading entry when Lynxtron artifacts are generated
...
- shared C++ N-API callback stubs under `shared/nativeModule/` for Lynxtron N-API native modules
...
Lynxtron native libraries use the shared Lynx C/C++ extension APIs and static registration macros. This page only covers how Autolink discovers and loads packages. For implementation details, use the Desktops(Node-API) tab for Native Modules and the Desktops tab for Custom Native Elements:

- [Native Modules for Desktops(Node-API)](/guide/use-native-modules?platform=node-api)
- [Custom Native Elements for Desktops](/guide/custom-native-component?platform=desktops)
