...
Use `pluginLynxtron()` in the Lynxtron host-side Rspack config. The plugin includes Lynxtron Autolink by default: it scans installed dependencies for `lynx.lib.json`, stages matching native library packages under the host output, and prepends generated code that requires each staged package's `./lynxtron` entry. For packages declared with `platforms.lynxtron.path`, direct imports of `<package>/lynxtron` are also aliased to a generated proxy that resolves the staged package at runtime. The `./lynxtron` entry loads the current host's `.node` artifact.
