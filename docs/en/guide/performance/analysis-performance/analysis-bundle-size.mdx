...
The two JS layers are encoded together into a single binary `.lynx.bundle`, located under `dist/` after a production build (see [Output Files](/build/output)). That binary is what the Lynx engine loads, and it is the final metric to watch when measuring size.
...
A development build also emits these intermediates under `dist/.lynx/`; see [Output Files](/build/output).
...
- Use [External Bundle](/build/external-bundle) to factor out reusable parts.
