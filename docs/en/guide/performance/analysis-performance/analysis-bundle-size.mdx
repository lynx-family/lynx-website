...
- **Reduce duplicate dependencies**: use Rsdoctor's duplicate-package detection to find dependencies bundled more than once, then converge them with [`resolve.dedupe`](/api/build/rspeedy.resolve.dedupe) or `pnpm dedupe`.
...
- Tune chunking with [`splitChunks`](/api/build/rspeedy.config.splitchunks) (which maps onto Rspack's [code splitting](https://rspack.rs/guide/optimization/code-splitting)).
- Use [External Bundle](/guide/build/external-bundle) to factor out reusable parts.

### Compile-layer options

- [`extractStr`](/api/build/react-rsbuild-plugin.pluginreactlynxoptions.extractstr): merges duplicate string literals to reduce size, especially effective for strings that are duplicated across the main-thread and background-thread layers, such as i18n text and class names.
