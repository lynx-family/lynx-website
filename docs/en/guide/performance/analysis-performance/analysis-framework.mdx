...
The instrumentation also has to be compiled into the bundle in the first place. It is added by default in development and removed by default in production; see [Runtime Profiling](/build/runtime-profiling.md) for how to turn it on for a production build.
