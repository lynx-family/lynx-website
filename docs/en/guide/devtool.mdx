...
You need to [integrate DevTool](/guide/start/integrate-lynx-devtool.mdx) into your Lynx pages, and then connect to the device via the [DevTool Desktop Application](#run-lynx-devtool-desktop-application) to debug the page.
...
Lynx DevTool can also be used by AI coding agents for automated inspection and debugging. Instead of manually operating the DevTool desktop application, agents can programmatically send CDP commands, read console logs, take screenshots, and more.

There are two integration options:

- **[DevTool Skill](/ai/skills#devtool)** — A CLI-based skill installed via `npx skills add lynx-community/skills`. Works with any coding agent (Claude Code, Cursor, Copilot).
- **[DevTool MCP](/ai/lynx-devtool-mcp)** — An MCP server for MCP-compatible clients.

## Compatibility

import { APITable } from '@lynx';

<APITable query="devtool/integration.connection" />
