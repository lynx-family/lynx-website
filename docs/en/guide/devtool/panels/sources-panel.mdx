...
import debugMetadataImg from '@assets/doc/devtool/sources/debug-metadata.png?url';
import debugMetadataUrlImg from '@assets/doc/devtool/sources/debug-metadata-url.png?url';
...
1. A `debug-metadata.json` file is generated in the intermediate output directory (`dist/.lynx/<entry>/`).
   <img
     class="margin"
     src={debugMetadataImg}
     alt="Debug metadata"
     style={{ height: 200 }}
   />
2. In the `tasm.json` file of the same directory, the `sourceContent.config.debugMetadataUrl` field is a valid URL pointing to the `debug-metadata.json` file mentioned in step one. Refer to [rspeedy.dev.assetprefix](/api/rspeedy/rspeedy.dev.assetprefix).
   <img
     class="margin"
     src={debugMetadataUrlImg}
