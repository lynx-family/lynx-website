...
After [configuring SourceMap](/api/build/rspeedy.output.sourcemap), you can directly debug the original code you author in the Sources panel.
...
1. A `debug-metadata.json` file is generated in the intermediate output directory (`dist/.lynx/<entry>/`).
   <img
     ...
     style={{ height: 200 }}
   />
2. In the `tasm.json` file of the same directory, the `sourceContent.config.debugMetadataUrl` field is a valid URL pointing to the `debug-metadata.json` file mentioned in step one. Refer to [rspeedy.dev.assetprefix](/api/build/rspeedy.dev.assetprefix).
   <img
     class="margin"
