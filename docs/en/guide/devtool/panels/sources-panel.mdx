...
1. A `debug-info.json` file is generated in the [intermediate output directory](/api/packages/rspeedy).
   <img
     class="margin"
     ...
     style={{ height: 200 }}
   />
2. In the `tasm.json` file of the [intermediate output directory](/api/packages/rspeedy), the `templateDebugUrl` field is a valid URL pointing to the `debug-info.json` file mentioned in step one. Refer to [rspeedy.dev.assetprefix](/api/config/#dev-assetprefix).
   <img
     class="margin"
