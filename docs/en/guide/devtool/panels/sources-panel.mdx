...
After [configuring SourceMap](/rspeedy/config/output#output-sourcemap), you can directly debug the original code you author in the Sources panel.
...
1. A `debug-info.json` file is generated in the [intermediate output directory](/rspeedy/api/).
   <img
     class="margin"
     ...
     style={{ height: 200 }}
   />
2. In the `tasm.json` file of the [intermediate output directory](/rspeedy/api/), the `templateDebugUrl` field is a valid URL pointing to the `debug-info.json` file mentioned in step one. Refer to [rspeedy.dev.assetprefix](/rspeedy/config/dev#dev-assetprefix).
   <img
     class="margin"
