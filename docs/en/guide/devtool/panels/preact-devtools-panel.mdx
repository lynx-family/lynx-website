...
Development builds (`rspeedy dev`) ship the runtime hooks the panel needs. Production builds (`rspeedy build`) strip them, along with `@lynx-js/preact-devtools` itself. To inspect a production build, set `REACT_DEVTOOL=true` when you build it:

```bash
REACT_DEVTOOL=true rspeedy build
```

:::info
`REACT_DEVTOOL` requires `@lynx-js/react-rsbuild-plugin@0.17.2` or later (which ships `@lynx-js/react-webpack-plugin@0.9.5`). Earlier versions always strip the hooks from production builds.
:::
