...
1. Build: turn on [`enableUiSourceMap`](#1-enable-it) so the build emits the
   UI source map into [`debug-metadata.json`](/guide/devtool/map-errors-to-source) and tags elements
   with a `nodeIndex`.
...
[Map Production Errors to Source → Model B](/guide/devtool/map-errors-to-source#model-b-also-point-the-bundle-at-your-host)).
