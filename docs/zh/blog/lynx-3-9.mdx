...
构建侧新增了双线程 Minimizer 机制，用于对主线程和后台线程产物进行更细粒度的压缩与裁剪。开发者可以通过 [`optimizeBundleSize`](/api/packages/react-rsbuild-plugin#optimizebundlesize) 快速启用默认优化，也可以分别配置 [`Minify.mainThreadOptions`](https://rsbuild.rs/zh/config/output/minify) 和 [`Minify.backgroundOptions`](https://rsbuild.rs/zh/config/output/minify)。
