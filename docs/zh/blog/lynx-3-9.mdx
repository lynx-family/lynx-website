...
在 React 中，[Portal](https://react.dev/reference/react-dom/createPortal) 用于把一段组件树渲染到元件树上的另一个容器里，同时让它仍然留在 React 树中——浮层、弹窗这类需要脱离当前布局层级的 UI 通常都靠它。自 `@lynx-js/react` 0.121.0 起，ReactLynx 通过 [`createPortal`](/react/api/functions#createportal) 提供 Portal。
...
Portal 只改变元件在页面上的物理位置。除此之外，渲染进 Portal 的 JSX 表现得就像渲染它的那个组件的子节点：父树提供的 React Context 依然能拿到，状态更新也和普通组件一致。有三点[需要注意](/react/api/functions#createportal备注)的地方与 Web 不同：
...
构建侧新增了双线程 Minimizer 机制，用于对主线程和后台线程产物进行更细粒度的压缩与裁剪。开发者可以通过 [`optimizeBundleSize`](/rspeedy/plugins/plugin-react#optimizebundlesize) 快速启用默认优化，也可以分别配置 [`Minify.mainThreadOptions`](/rspeedy/config/output#output-minify) 和 [`Minify.backgroundOptions`](/rspeedy/config/output#output-minify)。
