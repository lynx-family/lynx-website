...
我们将 JavaScript 社区广受欢迎的 UI 组件测试工具 [Testing Library](https://testing-library.com/) 引入到了 Lynx 中。新增的 [`@lynx-js/react/testing-library/`](/react/api/testing-library) 包提供了 [`render`](/react/api/testing-library#render) 等测试工具函数。同时，它也可以与官方的 [`@testing-library/jest-dom`](https://github.com/testing-library/jest-dom) 搭配使用，让你能够使用 `toBeInTheDocument` 等匹配器来验证元素是否存在及其行为表现。
