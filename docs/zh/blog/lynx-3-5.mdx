---
title: 'Lynx 3.5: 主线程脚本，React 编译器，HarmonyOS 能力对齐'
date: 2025-11-10
sidebar: false
authors: ['liujilong', 'huxpro', 'lynx']
badge_text: 'Lynx 3.5 发布了!'
description: 'Lynx 3.5 已经正式发布! 延续稳定的双月发布节奏，Lynx 3.5 带来了多项重要更新：React 编译器的实验性支持；主线程脚本支持了 `stopPropagation` 事件冒泡拦截、首帧即可交互、跨线程通信等一系列增强；进一步对齐了 HarmonyOS 平台能力，并新增了 `<pointer-events>` CSS 属性等功能。'
---
...
<h3 id="支持用-stoppropagation-阻止事件冒泡">
  支持用 <code>stopPropagation</code> 阻止事件冒泡 <Badge>Web 友好</Badge>
</h3>
...
此前，主线程事件需要等待后台线程完成[“激活”（Hydration）](https://lynxjs.org/3.5/zh/react/lifecycle.html)后才能响应，导致页面在首帧可见后，仍存在一段不可交互的延迟，类似[服务端渲染中的常见的恐怖谷效应](https://web.dev/articles/rendering-on-the-web#rehydration)。

为了消除这段延迟，我们优化了激活流程，使得主线程事件可以在首帧渲染后立刻响应，并在后台线程激活后正确接管主线程中的状态并重放 [`runOnBackground`](https://lynxjs.org/3.5/zh/react/api/functions#runonbackground)，在保证线程间的正确同步的同时，实现首帧可见即可交互。
...
Lynx 提供了 [`runOnMainThread`](https://lynxjs.org/3.5/zh/react/api/functions#runonmainthread) 和 [`runOnBackground`](https://lynxjs.org/3.5/zh/react/api/functions#runonbackground) 两个 API 用于跨线程函数调用。在 Lynx 3.5 中，这两个 API 现在支持异步返回值，让跨线程通信更加灵活和直观。
