...
与此同时，为拓展复杂场景下的能力边界，Lynx 3.4 也带来了一系列功能更新：我们优化了 `<list>` 组件的回收策略以提升渲染性能；提供了更完整流畅的动画支持；并加入了对可变字体的支持，允许开发者通过 font-variation-settings 等属性实现更精细的排版控制。
...
* 开发者可在 Windows 环境下成功编译安卓端的 Lynx SDK 及 [LynxExplorer](/guide/start/quick-start.html#准备-lynx-explorer) 应用；
