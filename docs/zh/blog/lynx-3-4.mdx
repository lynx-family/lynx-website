...
延续今年初公布的[技术路线图](/blog/lynx-open-source-roadmap-2025)，我们在新版本中开源了若干 Lynx 技术栈的核心能力：开源了对 HarmonyOS 的适配支持；支持 Windows 作为 Lynx 开发环境；并引入了用于提升开发效率的性能分析工具 Trace 与界面录制工具 Recorder，还新增了两个文本输入元件。

与此同时，Lynx 已在 TikTok 内部众多业务场景中广泛应用。为持续拓展复杂场景下的能力边界，Lynx 3.4 也带来了一系列功能更新：我们优化了 `<list>` 组件的回收策略以提升渲染性能；提供了更完整流畅的动画支持；并加入了对可变字体的支持，允许开发者通过 font-variation-settings 等属性实现更精细的排版控制。
...
Lynx 现已正式支持 HarmonyOS 平台。开发者可参考 [接入现有应用](/guide/start/integrate-with-existing-apps.html#platform=harmony)，将 Lynx 集成进 HarmonyOS 应用中。我们也将很快发布技术博客，深入介绍 Lynx 在 HarmonyOS 上的实现与优化，敬请期待。
...
* 开发者可在 Windows 环境下成功编译安卓端的 Lynx SDK 及 LynxExplorer 应用；

Lynx Devtool 桌面版对 Windows 的支持也在积极开发中，即将与大家见面。

## 新开发工具

Lynx 3.4 引入了两款开发工具，致力于提升开发者体验与调试效率。

### Trace

* Trace：能够深度追踪并可视化 Lynx 页面的完整渲染过程与引擎内部的执行逻辑，将以往难以洞察的“黑盒”运行状态清晰呈现。它不仅能帮助开发者快速理解 Lynx 的底层工作原理，更能大幅提升定位性能瓶颈的效率。

_这里缺一张 Trace 介绍的图_

### Recorder

* Recorder：可录制 Lynx 页面运行过程中的所有外部依赖（包括 bundle 资源、原生模块返回结果、用户交互序列等），并支持导出录制结果。在 LynxExplorer 中可完整回放所录制的场景，极大便利了复杂交互场景的复现与调试。

_这里缺一张 Recorder 介绍的图_

## 更多的内置元件

### 新增文本输入元件

在 Lynx 社区发起的 UI 能力调研中，`<Input>/<TextArea>` 以超过 50% 的得票率，成为开发者们最迫切期待的核心功能。
Lynx 3.4 交付了两个核心文本输入组件：

* [`<input>`](/api/elements/built-in/input): 单行文本输入元件
...
* [`<textarea>`](/api/elements/built-in/textarea): 多行文本输入元件
...
我们将继续完善核心元件，欢迎开发者们在 [discussion](https://github.com/lynx-family/lynx/discussions/673) 中继续投票，共同定义 Lynx 的未来。
...
若该改动对你现有场景造成影响，可通过设置 `experimental-recycle-sticky-item={false}` 暂时禁用此优化。如遇到任何问题，欢迎通过 issue 反馈。
...
* 当 `<list-item>` 内含复杂元件时，回收并重新更新节点带来较高的 CPU 开销。开发者现可设置 `recyclable={false}` 来禁用特定 `<list-item>` 的回收逻辑。禁用后，该条目滑出视窗后不会被回收，再次出现时也无需重新渲染，适用于对渲染性能有极致要求的场景。
