...
当与[主线程脚本](../../../react/main-thread-script.mdx)一起使用时，可以通过调用事件对象的 [`stopPropagation`](../../../api/lynx-api/event/event.mdx#stoppropagation) 方法来阻止事件冒泡。
...
在上面的示例中，点击内部元素不会触发外部元素的 `tap` 事件。
