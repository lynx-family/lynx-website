...
当需要传输大量数据时，服务端通常会采用 Streaming HTTP 的方式，将数据以流式形式逐步发送。此时，客户端可以利用 `fetch` 接口的流式处理能力，实时分批接收和处理数据，从而显著提升数据处理效率。

```typescript
fetch('https://sse.dev/test').then((response) => {
...
:::tip
试验性功能，页面中需要通过设置特定的 PageConfig `enableFetchAPIStandardStreaming = true`， 来开启 fetch 接口的流式读取能力。
:::
...
值得注意的是，Lynx 的 Fetch API 与 [Web Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) 相比存在细微差异。你可以查阅 [Fetch API 参考 - 兼容性](/api/lynx-api/global/fetch#兼容性)部分以了解更多信息。因此，你可能需要调整来自 Web 生态系统的库以确保兼容性。如果你在使用 Lynx Fetch API 时遇到任何问题，欢迎提交功能请求或通过[贡献](https://github.com/lynx-family/lynx/blob/develop/CONTRIBUTING.md)来帮助 Lynx 更好地支持 Web 生态系统。
