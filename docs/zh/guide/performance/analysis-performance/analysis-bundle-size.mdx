...
- **减少重复依赖**：用 Rsdoctor 的重复包检测找出被多份打包的依赖，再用 [`resolve.dedupe`](/config/#resolve-dedupe) 或 `pnpm dedupe` 收敛。
...
- 通过 [`splitChunks`](/config/split-chunks) 调整分包策略（底层即 Rspack 的 [代码分割](https://rspack.rs/zh/guide/optimization/code-splitting)）。
- 用 [External Bundle](/rspeedy/external-bundle) 把可复用的部分独立出去。

### 编译层选项

- [`extractStr`](/packages/react-rsbuild-plugin#extractstr)：合并重复的字符串字面量来减小体积，对 i18n 文案、类名等「在主线程与后台线程两层中重复」的字符串尤其有效。
