...
两份 JS 经过编码合成为一个二进制的 `.lynx.bundle`（生产构建后位于 `dist/`，详见 [构建输出文件](/build/output)），这才是 Lynx 引擎真正加载的交付产物，也是衡量体积时应当盯住的最终指标。
...
开发构建同样会在 `dist/.lynx/` 下生成这些中间产物，详见 [构建输出文件](/build/output)。
...
- 用 [External Bundle](/build/external-bundle) 把可复用的部分独立出去。
