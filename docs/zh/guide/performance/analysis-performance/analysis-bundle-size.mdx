...
- 用 [`'background only'` 指示符](/react/api/directives) 把一个函数体标记为只在后台线程保留。
- 用 [`__BACKGROUND__` / `__MAIN_THREAD__` 内置宏](/react/api/macros) 按线程裁剪代码块。
...
这是 Lynx 特有、也最该优先检查的一层。把只在后台需要的代码（请求 / NativeModule 调用 / 日志 / 监控 SDK）用 [`'background only'`](/react/api/directives) 或 [`__BACKGROUND__`](/react/api/macros) 移出渲染路径，避免它被打包进 `main-thread.js`。若这类代码来自你无法修改的第三方库，则需要推动库侧在定义处加上标注。

### 精简后台线程产物

- **减少重复依赖**：用 Rsdoctor 的重复包检测找出被多份打包的依赖，再用 [`resolve.dedupe`](/rspeedy/config/resolve#resolve-dedupe) 或 `pnpm dedupe` 收敛。
...
- 通过 [`splitChunks`](/rspeedy/config/splitChunks#splitchunks) 调整分包策略（底层即 Rspack 的 [代码分割](https://rspack.rs/zh/guide/optimization/code-splitting)）。
- 用 [External Bundle](/rspeedy/external-bundle) 把可复用的部分独立出去。

### 编译层选项

- [`extractStr`](/rspeedy/plugins/plugin-react#extractstr)：合并重复的字符串字面量来减小体积，对 i18n 文案、类名等「在主线程与后台线程两层中重复」的字符串尤其有效。
