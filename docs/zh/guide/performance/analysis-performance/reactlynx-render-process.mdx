...
[首帧渲染](/guide/spec.html#first-frame-rendering-or-ffr)包括下载 [Bundle](/guide/spec.html#lynx-bundle-or-bundle)、加载 Bundle、绘制等阶段。
...
[嵌套 text](/api/elements/built-in/text.html#嵌套-text) 会和父节点 text 合并，创建一个平台层 UI。
:::
...
为了在 Trace 中更清晰的看到点击事件回调的执行时机，可以使用 [Trace API](/guide/performance/analysis-performance/trace-api) 添加自定义 Trace 事件。
...
为了在 Trace 中更清晰的看到 useEffect 回调的执行时机，使用 [Trace API](/guide/performance/analysis-performance/trace-api) 添加自定义 trace。
