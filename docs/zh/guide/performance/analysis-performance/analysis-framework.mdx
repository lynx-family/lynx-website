...
### 把埋点编译进产物 \{#compiling-the-instrumentation-into-the-bundle}

ReactLynx 为组件的 `render` 和 `diff` 内置了 Trace 点，用来观察组件如何渲染、如何比较差异。

![react profile](https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/plugin/static/rspeedy-react-profile.png)

- 开发环境（`npx rsbuild dev`）默认**添加**这些 Trace 点。
- 生产环境（`npx rsbuild build`）默认**移除**这些 Trace 点。

#### 在生产构建中打开

把 `performance.profile` 设为 `true`。想优化应用性能时，这很有用。
...
:::tip
可以用 [`npx rsbuild preview`](https://rsbuild.rs/zh/guide/basic/cli#rsbuild-preview) 在本地预览产物。
:::

:::warning
**不要**把 `performance.profile: true` 的产物发到线上，它们不适用于生产环境。
:::

#### 在开发环境中关闭

把 `performance.profile` 设为 `false`。
...
用 [JS Profile](/guide/devtool/trace/js-profile) 在运行时采集调用堆栈，不需要改构建配置。
