...
反之，如果页面依赖额外的数据才能完成显示，例如通过网络下载或本地文件读取等异步方式获取的数据来“真正”完成渲染，那么可以[标记 Lynx 流水线](guide/performance/timing-flag)。标注后，可以获取被标记的 [Lynx Pipeline](guide/spec#lynx-pipeline) 产生的 _ActualFMP_。与无数据请求时不同，这种情况下 FCP 和 TTI 完成时可能展现的是无意义的画面，例如加载动画或骨架屏。
...
由此你也可以发现，显示真实数据需要消耗额外的处理时间，这与提升用户感知性能的基本准则相悖。因此为了提高用户感受到的性能，首屏展示的内容应该尽可能是静态内容或者预加载内容，其余内容再依赖网络下载并在后续的数据更新中显示。这种方式虽然增加了总的数据下载量，但是通过更早渲染有意义的内容可以极大提升用户感受到的性能。
...
从用户点击链接到新的页面显示，应用程序需要完成包括页面容器初始化、准备 [TemplateBundle](api/lynx-native-api/template-bundle) 等初始化工作。
...
不同的业务目标意味着你可能需要关注不同的性能指标。对[性能 API](api/lynx-api/performance-api) 的使用可以不局限于通过内置指标来分析页面性能，你也可以灵活组合不同 [`PerformanceEntry`](api/lynx-api/performance-api/performance-entry) 提供给你的关键节点的时间，构建出一套适配你的应用程序的性能检测指标。
