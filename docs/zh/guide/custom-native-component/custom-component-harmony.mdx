...
下面是 `<input>` 自定义元件的实现，需要继承自 `UIBase`。

```objc title="LynxExplorerInput.ets" {20-23}
...
export function buildInput(ui: UIBase) {
  if (ui as LynxExplorerInputView) {
    InputView();
  }
}

export class LynxExplorerInputView extends UIBase {
...
export class CustomElement {
  ...
  static initialize() {
    ...
    BUILTIN_BEHAVIORS.set("input", new Behavior(LynxExplorerInput, undefined));
...
import { LynxExplorerInputView } from './input/LynxExplorerInputView';

build() {
    LynxView({
      ...
      behaviors: new Map([['input', new Behavior(LynxExplorerInputView, undefined)]]),
      ...
    }).height('100%')
  ...
}
...
```objc title="LynxExplorerInput.ets" {31-37,43}
@ObservedV2
class InputParams {
  constructor(ui: LynxExplorerInputView) {
    this.ui = ui;
  }

  @Trace inputText: string = '';
  @Trace placeholder: string = '';
  ui: LynxExplorerInputView;
}
...
export function buildInput(ui: UIBase) {
  if (ui as LynxExplorerInputView) {
    InputView({ inputParams: (ui as LynxExplorerInputView).inputParams });
  }
}
...
你可以通过继承并重写 `update(props: Object, events?: EventHandlerArray[])` 来监听前端传入的属性变化，并更新原生视图。例如，处理 `<input>` 元件的 value 属性：

<CodeFold toggle height={200}>
```objc title="LynxExplorerInput.ets" {8}
...
ui.inputParams.placeholder = entry[1] as string;
} else if (entry == 'value') {
ui.inputParams.inputText = entry[1] as string;
} else {
...
}
}
...
```objc title="LynxExplorerInput.ets"  {12}
...
```objc title="LynxExplorerInputShadowNode.ets"  {4,7,10}
...
```objc title="LynxExplorerInputShadowNode.ets"  {10}
...
```objc title="LynxExplorerInputShadowNode.ets"  {20}
...
返回值：
返回值是一个数组，需要返回3个值：宽度、高度、Baseline (影响 vertical 对齐，默认传0）
...
```objc title="LynxExplorerInputShadowNode.ets"  {8,11}
...
<CodeFold toggle height={200}>
```objc title="LynxExplorerInput.ets" {13}
import { UIBase, EventHandlerArray, LynxUIMethodConstants } from '@lynx/lynx';
...
@Builder
export function buildInput(ui: UIBase) {
...
```objc title="LynxExplorerInput.ets" {16-25}
...
```objc title="LynxExplorerInput.ets" {15}
