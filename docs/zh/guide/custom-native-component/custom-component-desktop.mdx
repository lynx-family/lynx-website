...
自定义元件的实现分为几个步骤，包括：声明并注册元件、创建原生视图、处理布局、在 Lynx 页面中使用等。Desktop 平台通过 Lynx C/C++ API 实现原生元件。接下来以一个简单的自定义输入框元件 `<explorer-input>` 为例，介绍自定义元件的实现流程。
...
把所有 Desktop backend 共享的行为放在 element header 中。各平台 backend 继承这个类，只实现展示相关的部分。
...
Desktop 原生元件通过 `LYNX_REGISTER_ELEMENT` 注册。这是一次进程级的静态注册：原生库被编译进宿主应用或被宿主进程加载后，进程内每个 `LynxView` 都可以使用注册的 tag。
...
每个自定义元件都会被创建为一个 `LynxNativeView`。上面的声明、属性处理、方法处理和静态注册对所有 Desktop 实现都是共享的。backend 决定 view 创建之后持有什么：Native UI 持有平台控件，Texture 持有 Lynx surface。

下面的示例使用同一个注册 creator 名称展示两种可选 backend 实现。实际构建时选择一种 backend，让 `CreateExplorerInputElementNativeView` 只解析到一个实现。

<Tabs groupId="custom-component-desktop-backend">

<Tab label="Native UI">

Native UI backend 会 attach 一个平台控件，并在 `OnLayoutChanged` 中更新 frame。对于 `<explorer-input>`，实现会 attach 一个最小输入控件。
...
用 texture 实现文本输入是可行的，但会涉及文本编辑、光标、选区和 IME 处理，示例会很长。下面的示例只聚焦 texture 链路：绘制一块原生纹理，并复用 `ExplorerInputElementView` 中共享的 `value` 属性和 `setValue` UI method。

共享的 texture backend 是跨平台的。它负责打开 surface 路径、维护布局尺寸，并调用各平台 backend 实现的 `DrawExplorerInputElementTextureSurface`。
...
准备好原生代码后，将它编译进宿主应用或可被宿主加载的 Desktop 原生库。如果包里同时包含 Native UI 和 Texture 两套实现，需要在构建系统中选择一种，让注册的 creator 只解析到一个实现。

原生库被 link 或加载后，即可在 Lynx 中使用注册好的 tag：
...
在某些情况下，前端可能需要通过命令式 API 直接操作自定义元件。Desktop 上可以通过实现 `OnMethodInvoked` 处理这些调用。

#### 前端调用示例

以下代码展示了如何在前端通过 [SelectorQuery](/api/lynx-api/selector-query) 调用 `setValue` 方法更新 `<explorer-input>` 元件：
