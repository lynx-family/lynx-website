...
自定义元件的实现分为几个步骤，包括：声明并注册元件、创建原生视图、处理布局、在 Lynx 页面中使用等。Desktops 平台通过 Lynx C/C++ API 实现原生元件。接下来以一个简单的自定义输入框元件 `<explorer-input>` 为例，介绍自定义元件的实现流程。
...
把所有 Desktops backend 共享的行为放在 element header 中。各平台 backend 继承这个类，只实现展示相关的部分。
...
每个自定义元件都会被创建为一个 `LynxNativeView`。上面的声明、属性处理、方法处理和静态注册对所有 Desktops 实现都是共享的。backend 决定 view 创建之后持有什么：Native UI 持有平台控件，Texture 持有 Lynx surface。
...
准备好原生代码后，将它编译进宿主应用或可被宿主加载的 Desktops 原生库。如果包里同时包含 Native UI 和 Texture 两套实现，需要在构建系统中选择一种，让注册的 creator 只解析到一个实现。
