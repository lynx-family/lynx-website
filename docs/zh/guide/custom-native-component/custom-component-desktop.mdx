...
自定义元素的实现分为几个步骤，包括：声明并注册元素、创建原生视图、处理布局、在 Lynx 应用代码中使用等。Desktop 平台通过 Lynx C/C++ API 实现原生元素。接下来以一个简单的自定义输入框元素 `<explorer-input>` 为例，介绍自定义元素的实现流程。

下文用到的扩展头文件，包括 `capi/lynx_view_capi.h`、`lynx_native_view.h` 和 `lynx/registration.h`，由 npm 包 [`@lynx-js/lynx-library-headers`](https://www.npmjs.com/package/@lynx-js/lynx-library-headers) 提供。在原生库工程中安装该包，把它的 `include/` 目录加入头文件搜索路径即可；由 [`create-lynx-library`](/guide/autolink?platform=lynxtron) 生成的 CMake 已经完成了这一步。
...
把所有 Desktop 渲染后端共享的行为放在元素头文件中。各平台渲染后端继承这个类，只实现展示相关的部分。
...
Desktop 原生元素通过 `LYNX_REGISTER_ELEMENT` 注册。这是一种进程级静态注册：原生库被编译进宿主应用或被宿主进程加载后，静态注册会让该标签成为 Lynx 运行时中可用的 Lynx 元素。
...
每个自定义元素都会被创建为一个 `LynxNativeView`。上面的声明、属性处理、方法处理和静态注册对所有 Desktop 实现都是共享的。渲染后端决定视图创建之后持有什么：Native UI 持有平台控件，Texture 持有 Lynx 管理的 Surface。
...
准备好原生代码后，将它编译进宿主应用或可被宿主加载的 Desktop 原生库。如果包里同时包含 Native UI 和 Texture 两套渲染后端实现，需要在构建系统中选择一种，让注册创建函数只解析到一个实现。
...
在某些情况下，前端可能需要通过命令式 API 直接操作自定义元素。Desktop 上可以通过实现 `OnMethodInvoked` 处理这些调用。
