...
<Details title="这三个开关均默认关闭，需要主动打开">
...
dependencies {
  ...
  implementation "org.lynxsdk.lynx:lynx-service-devtool:3.5.2"
}
```
...
- 打开 `Lynx Debug` 后，`Lynx DevTool` 和 `Lynx LogBox` 开关才能够生效
- 使用 DevTool 桌面应用调试 Lynx 页面时，需要打开 `Lynx DevTool` 开关
  - 如果需要使用 JavaScript 调试功能，还需要设置 `LoadJsBridge` 预设值为 `true`
- LogBox 可以帮助你快速发现问题
