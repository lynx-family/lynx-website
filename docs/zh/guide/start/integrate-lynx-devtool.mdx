---
description: '接入 Lynx DevTool 进行页面调试'
---
...
- DevTool 处于 `ENABLED` 状态后，`Lynx DevTool` 和 `Lynx LogBox` 开关才能够生效
...
- DevTool 处于 `ENABLED` 状态后，`Lynx DevTool` 和 `Lynx LogBox` 开关才能够生效
- 使用 DevTool 桌面应用调试 Lynx 页面时，需要打开 `Lynx DevTool` 开关
  - 如果需要使用 JavaScript 调试功能，还需要设置 `LoadQJSBridge` 或 `LoadV8Bridge` 启动配置值为 `true`
...
<Details title="LogBox 默认开启，DevTool 默认关闭。建议开启 DevTool 开关">
