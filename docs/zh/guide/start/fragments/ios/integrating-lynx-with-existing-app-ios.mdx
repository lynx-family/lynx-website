### 将 Lynx 集成到 iOS 平台 \{#integrate-lynx-with-ios-platform,platform=ios}
...
## 1. 依赖配置 \{#dependency-configuration,platform=ios}
...
### 配置依赖 \{#configuring-deps,platform=ios}
...
### 安装依赖 \{#install-dependencies,platform=ios}
...
## 2. Lynx 环境初始化 \{#lynx-env-initialization,platform=ios}

<Steps>

### LynxEnv 初始化 \{#lynx-env-initialization,platform=ios}
...
## 3. 渲染 LynxView \{#render-lynxview,platform=ios}
...
### 创建 Bundle 加载器 \{#create-bundle-loader,platform=ios}
...
### 构造基础 LynxView \{#construct-basic-lynxview,platform=ios}
...
### 将 LynxView 添加到视图 \{#add-lynxview-to-the-window,platform=ios}
...
## 4. 进入 Lynx 世界 \{#now-what,platform=ios}
