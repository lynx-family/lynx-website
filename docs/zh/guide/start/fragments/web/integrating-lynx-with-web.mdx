### 将 Lynx 集成到 Web 平台 \{#integrate-lynx-with-web-platform,platform=web}
...
## 1. 构建 web 产物 \{#build-web-artifact,platform=web}

我们需要你已经阅读并按照[快速开始](/guide/start/quick-start)创建了一个 Lynx 项目。

<Steps>

### 添加 web 配置 \{#add-web-configuration,platform=web}
...
## 2. 接入 web 项目 \{#integrate-web-project,platform=web}

目前你已经有了一份 Lynx for Web 构建产物，接下来我们需要创建一个 web 工程去使用它，这里使用 Rsbuild。

<Steps>

### 创建一个 web 项目 \{#create-web-project,platform=web}
...
### 配置工程 \{#configure-project,platform=web}
...
### 启动工程 \{#start-project,platform=web}
...
## 3. API 参考 \{#api-reference,platform=web}
...
#### 3.5 宽、高 \{#lynx-view-width-height,platform=web}
...
### 3.2 兼容性 \{#compatibility,platform=web}
...
#### 3.3.1 运行时报错：`Uncaught SecurityError: Failed to construct 'Worker': Script at 'xxx' cannot be accessed from origin 'xxx'`. \{#runtime-error,platform=web}
...
#### 3.3.2 性能优化 \{#performance-optimization,platform=web}
