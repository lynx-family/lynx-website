...
<Info title="Lynx for Android">
  ...
  - 下文中的所有代码，你都可以参考项目: <Link href={'https://github.com/lynx-family/integrating-lynx-demo-projects/tree/' + (versionJson.current_version === 'next' ? 'main': `release/${versionJson.current_version}`)}>integrating-lynx-demo-projects</Link>
</Info>
...
dependencies {
    ...
    // 仅在模板使用 <svg> 时需要；servalsvg 提供 SVG 渲染能力。
...
### 引入 XElement
XElement 需要额外在`LynxViewBuilder `中被引入：
