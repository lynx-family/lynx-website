### 将 Lynx 集成到 Android 平台 \{#integrate-lynx-with-existing-apps-android,platform=android}
...
## 1. 依赖配置 \{#dependency-configuration,platform=android}

<Steps>

### 包依赖 \{#configuring-gradle,platform=android}
...
### 配置混淆规则 ([Proguard](https://developer.android.com/build/shrink-code)) \{#configure-obfuscation-rules,platform=android}
...
## 2. 环境初始化 \{#environment-initialization,platform=android}

<Steps>

### LynxService 初始化 \{#lynx-service-initialization,platform=android}
...
### LynxEnv 初始化 \{#lynx-env-initialization,platform=android}
...
## 3. 渲染 LynxView \{#render-lynxview,platform=android}

<Steps>

### 创建 Bundle 加载器 \{#create-bundle-loader,platform=android}
...
### 构造 LynxView \{#construct-lynxview,platform=android}
...
### 引入 XElement \{#introduce-xelement,platform=android}
...
### 渲染视图 \{#render-view,platform=android}
...
## 4. 进入 Lynx 世界 \{#now-what,platform=android}
