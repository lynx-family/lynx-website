import { Go, Tip, Details } from '@lynx';
...
你可以使用 [`width`]、[`height`]、[`max-width`]、[`min-width`]、[`max-height`]、[`min-height`]、[`margin`]、[`padding`] 和 [`border-width`] 描述元件大小与盒模型，这些属性支持 `px`、`%`、`vh` 等多种[长度单位](/api/css/data-type/length.html#单位)。同时 `width` 和 `height` 支持 [`fit-content`] 和 [`max-content`] 的取值，用于元件的大小根据其内容自行撑开。
...
<Tip title={`启用 CSS 属性继承和逻辑属性`}>
  请参考 [CSS 继承](/guide/styling/custom-theming#按需使用-css-继承), 使用
  [`enableCSSInheritance`] 配置，允许 `direction` 属性被继承，以及使用依赖于
  `direction` 的逻辑属性。
</Tip>
...
[`enableCSSInheritance`]: /api/build/react-rsbuild-plugin.pluginreactlynxoptions.enablecssinheritance#pluginreactlynxoptionsenablecssinheritance-property
