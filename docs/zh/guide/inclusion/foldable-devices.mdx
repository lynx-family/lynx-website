...
运行时更新 GlobalProps，传入的字段会合并到已有数据中，并触发 [`onGlobalPropsChanged`](/api/react/global-events)。在默认的 [`globalPropsMode: 'reactive'`](/api/packages/react-rsbuild-plugin#globalpropsmode) 下，**ReactLynx 会自动对整个页面重新执行 render**，组件在 render 时读取最新尺寸即可。如果选择 `'event'` 模式，则不会自动触发整页 render。
