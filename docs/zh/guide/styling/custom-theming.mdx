...
需要配置 [`enableCSSInheritance`](/api/build/react-rsbuild-plugin.pluginreactlynxoptions.enablecssinheritance) 来启用继承能力。
...
除了默认可继承属性之外，可以在页面配置 [`customCSSInheritanceList`](/api/build/react-rsbuild-plugin.pluginreactlynxoptions.customcssinheritancelist) 自定义可继承的 CSS 属性。
