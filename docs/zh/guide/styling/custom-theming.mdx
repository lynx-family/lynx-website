...
需要配置 [`enableCSSInheritance`](/rspeedy/plugins/plugin-react#enablecssinheritance) 来启用继承能力。
...
除了默认可继承属性之外，可以在页面配置 [`customCSSInheritanceList`](/rspeedy/plugins/plugin-react#customcssinheritancelist) 自定义可继承的 CSS 属性。
