...
打开 [DevTool 开关页面](/guide/start/integrate-lynx-devtool-advanced.html#debugging-devtool-switch)，将“**V8 引擎**”切换为“**On**”并重启应用，即可在后台线程使用 V8 引擎。你可以在[预览窗口](/guide/devtool/panels/elements-panel#预览窗口介绍)的左下角查看当前的引擎类型。
...
1. 点击 **Customize And Control DevTools** <InlineIcon src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/lynx-website/assets/sources/more-options.png" alt="更多选项" /> > **Search**，打开**搜索**面板。
...
- 点击 **Match Case** <InlineIcon src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/lynx-website/assets/sources/match-case.png" alt="区分大小写" /> 使查询区分大小写。
- 点击 **Use Regular Expression** <InlineIcon src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/lynx-website/assets/sources/use-regular.png" alt="使用正则表达式" /> 以使用正则表达式进行搜索。
- 点击 **Refresh** <InlineIcon src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/lynx-website/assets/sources/refresh.png" alt="刷新" /> 来重新搜索。
- 点击 **Clear** <InlineIcon src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/lynx-website/assets/sources/clear.png" alt="清空" /> 来清空输入内容。
...
[配置 SourceMap](/api/config/output/source-map) 后，你可以直接在 Sources 面板中调试你编写的原始代码。
...
1. 在中间产物目录（`dist/.lynx/<entry>/`）下，生成了 `debug-metadata.json` 文件。
   <img
     class="margin"
     ...
     style={{ height: 200 }}
   />
2. 在同一目录的 `tasm.json` 文件中，`sourceContent.config.debugMetadataUrl` 字段是一个有效的 URL，其指向步骤一中提到的 `debug-metadata.json` 文件。参考 [rspeedy.dev.assetprefix](https://rsbuild.rs/zh/config/dev/asset-prefix)。
   <img
     class="margin"
