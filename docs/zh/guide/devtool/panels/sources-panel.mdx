...
1. 在[中间产物目录](/api/packages/rspeedy)下，生成了 `debug-info.json` 文件。
   <img
     class="margin"
     ...
     style={{ height: 200 }}
   />
2. 在[中间产物目录](/api/packages/rspeedy)的 `tasm.json` 文件中，`templateDebugUrl` 字段是一个有效的 URL，其指向步骤一中提到的 `debug-info.json` 文件。参考 [rspeedy.dev.assetprefix](/api/config/#dev-assetprefix)。
   <img
     class="margin"
