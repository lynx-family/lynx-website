...
1. 在中间产物目录（`dist/.lynx/<entry>/`）下，生成了 `debug-metadata.json` 文件。
   <img
     ...
     style={{ height: 200 }}
   />
2. 在同一目录的 `tasm.json` 文件中，`sourceContent.config.debugMetadataUrl` 字段是一个有效的 URL，其指向步骤一中提到的 `debug-metadata.json` 文件。参考 [rspeedy.dev.assetprefix](/api/rspeedy/rspeedy.dev.assetprefix)。
   <img
     class="margin"
