...
[配置 SourceMap](/rspeedy/config/output#output-sourcemap) 后，你可以直接在 Sources 面板中调试你编写的原始代码。
...
1. 在[中间产物目录](/rspeedy/api/)下，生成了 `debug-info.json` 文件。
   <img
     class="margin"
     ...
     style={{ height: 200 }}
   />
2. 在[中间产物目录](/rspeedy/api/)的 `tasm.json` 文件中，`templateDebugUrl` 字段是一个有效的 URL，其指向步骤一中提到的 `debug-info.json` 文件。参考 [rspeedy.dev.assetprefix](/rspeedy/config/dev#dev-assetprefix)。
   <img
     class="margin"
