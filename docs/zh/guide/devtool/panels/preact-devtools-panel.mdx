...
开发构建（`rspeedy dev`）默认就带上了面板所需的运行时钩子。生产构建（`rspeedy build`）默认会把它们连同 `@lynx-js/preact-devtools` 一起裁掉，如果你要调试一个生产构建的产物，构建时加上 `REACT_DEVTOOL=true`：
...
:::info
`REACT_DEVTOOL` 需要 `@lynx-js/react-rsbuild-plugin@0.17.2` 及以上（对应 `@lynx-js/react-webpack-plugin@0.9.5`）。更早的版本会在生产构建中无条件裁掉这些钩子。
:::
