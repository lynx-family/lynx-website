...
在 Lynxtron host 侧 Rspack 配置中使用 `pluginLynxtron()`。这个插件默认包含 Lynxtron Autolink：它会扫描已安装依赖中的 `lynx.lib.json`，把匹配到的 native library package stage 到 host 输出目录，并前置生成代码 require 每个 staged package 的 `./lynxtron` 入口。对于声明了 `platforms.lynxtron.path` 的 package，业务代码直接 import `<package>/lynxtron` 时，也会被 alias 到生成的 proxy，并在运行时解析 staged package。`./lynxtron` 入口负责加载当前 host 对应的 `.node` 产物。
...
Lynxtron 加载原生库后，其中的 Lynx 静态注册会对 Lynx UI 生效。库作者不需要 Lynxtron 专用注册宏。
...
Lynxtron 原生库使用共享的 Lynx C/C++ extension API 和静态注册宏。本页只说明 Autolink 如何发现和加载包；原生模块请看 Desktops(Node-API) tab，自定义元件请看 Desktops tab：
