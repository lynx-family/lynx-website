...
库应使用 Lynx 原生注解和注册宏。原生模块通常继承 `lynx-autolink-codegen` 生成的 spec；元件和 Service 会通过原生标记被发现。
...
Autolink 对外使用 Lynx 原生标记名称作为库作者 API。

对于 iOS 包，Autolink 会扫描 `@LynxNativeModuleRegister(...)`、`@LynxUIRegister(...)` 和 `@LynxServiceRegister(...)` 声明，让这些包可以通过生成的 registry 被链接进来。
