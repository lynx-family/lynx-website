...
在包含 HarmonyOS 工程的 npm 项目中安装 Hvigor plugin：

```bash
npm install --save-dev @lynx/lynx-library-plugin
```

在 entry HAP 的 `hvigorfile.ts` 中启用一次：
...
Hvigor 配置阶段，plugin 会扫描当前工程可见的 `node_modules`，在 HAP 模块的 `build/generated/lynx-autolink` 下生成 Registry HAR 和 AppStartup 任务。plugin 通过 Hvigor 的 project/HAP model API 把 library HAR 和 Registry HAR 加入构建图，注入 Registry 依赖与生成资源目录，并设置 `module.appStartup`。生成的 profile 直接引用 `build/generated` 下的任务，整个过程不会修改应用的 `src` 目录。
