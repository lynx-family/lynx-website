...
Hvigor 创建模块节点前，plugin 会扫描当前工程可见的 `node_modules`，使用 Hvigor 公开的 `parseJsonFile()` API 解析 HarmonyOS JSON5 元数据，在工程已忽略的 `.hvigor/lynx-autolink/<moduleName>` 缓存目录中生成 Registry HAR，并通过 `hvigorConfig.includeNode()` 把 library HAR 和 Registry HAR 加入构建图。目标 HAP 加载完成后，plugin 再通过 HAP model API 注入 Registry 依赖、生成资源目录与 `module.appStartup`。AppStartup 源码保存在 HAP 模块的 `build/generated/lynx-autolink` 下；plugin 会注册一个 Hvigor 任务，在 `clean` 之后、当前 target 的 `PreBuild` 之前重新生成这些文件。整个过程不会修改应用的 `src` 目录或已提交的 `build-profile.json5`。
