...
下文的 `targets` 清单和原生产物自动打包已包含在 `@lynx-js/create-lynxtron`、`@lynx-js/lynxtron`、`@lynx-js/lynxtron-dev-plugins` 和 `@lynx-js/lynxtron-builder` 的 `0.0.22` 正式发布版本中。库创建流程已使用 `create-lynx-library@0.6.1` 和 `@lynx-js/autolink-codegen@0.6.0` 验证。复现本教程时，请配套使用这些版本并保留锁文件。
...
这些路径只标识 npm 包内的源产物。Lynxtron 的开发插件和 builder 会自动决定它们在开发输出及最终应用包中的位置；应用团队不需要复制这些产物，也不需要为每个库增加打包规则。Lynxtron 清单不提供不透明的 package 根目录或旧字段别名：请严格使用 `targets`、`files`、`frameworks`、`appBundles` 和 `arch` 选择器。
