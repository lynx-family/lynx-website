...
Lynx 原生库是一个 npm 包，可以打包元件、原生模块和 Service 中的任意组合，供宿主 Lynx 应用消费。Autolink 是一种集成机制，让宿主应用自动发现 `node_modules` 中安装的库，并接入它们的原生能力，避免为每个元件、原生模块或 Service 手动写注册代码。
...
- Lynxtron：`create-lynx-library` 可以生成 shared native library 包，`@lynx-js/lynxtron-dev-plugins` 通过 `pluginLynxtron()` 提供 host 侧加载能力。
...
应用侧像安装普通 npm 依赖一样安装库。Android Gradle plugin 和 iOS CocoaPods plugin 会读取 `lynx.lib.json`，把原生代码链接进宿主应用；宿主应用本身无需了解每个库具体暴露了哪些能力。Lynxtron 会从已安装包中读取同一份 manifest，把匹配到的包文件 stage 到 host 输出目录，并在 host bundle 前置生成代码 require 每个 staged package 的 `./lynxtron` 入口。
...
- Lynxtron 接入需要应用具备 host 侧 Rspack 构建，并通过 `pluginLynxtron()` 安装和加载原生库；一般消费编译后的 `dist/` 产物。
...
在 Lynxtron 宿主的 Rspack 配置里加入 `pluginLynxtron()`。该插件默认包含 Lynxtron Autolink：它会扫描已安装依赖中的 `lynx.lib.json`，把匹配到的原生库包复制到宿主产物目录，并在宿主 bundle 中注入代码，引入每个复制过来的包的 `./lynxtron` 入口。对于声明了 `platforms.lynxtron.path` 的包，业务代码里直接 `import '<package>/lynxtron'` 也会被别名到一个自动生成的代理模块，在运行时解析已复制的包。`./lynxtron` 入口负责加载当前宿主对应的 `.node` 产物。
...
`entry` 是 Lynxtron 宿主自身的 bundle 入口，`pluginLynxtron()` 会以它为基准放置复制过来的原生库包。完整的参数含义请参考 `@lynx-js/lynxtron-dev-plugins` 包。

Lynxtron 加载原生库后，其中的 Lynx 静态注册即对 Lynx UI 生效。库作者不需要 Lynxtron 专用的注册宏。
...
对于 Lynxtron，`platforms.lynxtron.path` 声明原生产物根目录，通常是 `dist`。`pluginLynxtron()` 会 stage 匹配到的包文件并校验产物；包的 `./lynxtron` export 负责加载当前 host 的 `.node`，例如 `dist/macos/arm64/lynx-button.node` 或 `dist/windows/x64/lynx-button.node`。
...
一个典型的库结构如下；`shared/` 用于跨端原生实现，如果同一个包也发布 Lynxtron 原生产物，同一个结构中也可以包含 Lynxtron loading 入口和分 host 输出：
...
- `shared/` 放置跨端原生代码，例如跨端自渲染 element 实现，以及 `shared/nativeModule/` 下的 C++ N-API callback stubs。
- `lynxtron/` 和 `dist/` 放置 Lynxtron loading 入口和分 host 输出。
...
- 生成 Lynxtron 产物时使用的 `lynxtron/` loading 入口
...
- N-API native module：`shared/nativeModule/` 下的 C++ N-API callback stubs
...
在原生源码中使用平台对应的 Lynx 注解或注册方式，Autolink 就能发现库提供的能力。Android 和 iOS 原生模块通常继承 `lynx-autolink-codegen` 生成的 spec；元件和 Service 会通过原生标记被发现。
...
Lynxtron 原生库使用共享的 Lynx C/C++ 扩展 API 和静态注册宏。本页只说明 Autolink 如何发现和加载包；原生模块的实现请看 Desktop (Node-API) 标签页，自定义元件的实现请看 Desktop 标签页：

- [原生模块 Desktop (Node-API) 说明](/guide/use-native-modules?platform=node-api)
- [自定义元件 Desktop 说明](/guide/custom-native-component?platform=desktops)
...
Autolink 在构建期运行。它会扫描 `node_modules` 中每个 npm 包的 `lynx.lib.json` manifest，然后为当前平台生成或提供 host 接入数据。Android 侧的 registry 生成由 Gradle settings plugin 和 build plugin 配合完成；iOS 侧由 CocoaPods plugin 完成并产出 registry pod。宿主应用在初始化 `LynxEnv` 时会一次性加载生成的 registry，因此 Android 或 iOS 宿主代码中不需要任何逐库的接入逻辑。Lynxtron Autolink 运行在 host Rspack 构建中：它会 stage 匹配到的 native package，把 loader 注入 host bundle，并让已加载库里的 Lynx 静态注册提供对应的原生模块和元件。
