...
生成的 AppStartup task 会在应用代码创建 Lynx 运行时之前于主线程运行，并且只调用 Registry HAR 中幂等的 `setupGlobal()`。如果 HAP 已配置 AppStartup profile，插件会保留原有 task 和 `configEntry`，并追加 Lynx task。应用无需运行 `lynx-harmony-autolink` 命令，也无需手动调用 `setupGlobal()`。

</PlatformTabs.Tab>

<PlatformTabs.Tab platform="lynxtron">

Lynxtron 为 Desktop 原生库提供了 Autolink 支持。并不是所有嵌入 Lynx 的 Desktop 宿主都开箱支持 Autolink，这是 Lynxtron 自带的能力。
...
Lynxtron 加载原生库后，其中的 Lynx 静态注册会在 Lynx 运行时中生效。库作者不需要 Lynxtron 专用注册宏。
...
Lynxtron 原生库使用共享的 Lynx C/C++ 扩展 API 和静态注册宏。本页只说明 Autolink 如何发现和加载包；原生模块的实现请看 Desktop (Node-API) 标签页，自定义元素的实现请看 Desktop 标签页：

- [原生模块 Desktop (Node-API) 说明](/guide/use-native-modules?platform=node-api)
- [自定义元素 Desktop 说明](/guide/custom-native-component?platform=desktops)
