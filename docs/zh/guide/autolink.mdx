...
Hvigor 创建模块节点前，plugin 会扫描当前工程可见的 `node_modules`，在 HAP 模块的 `build/generated/lynx-autolink` 下生成 Registry HAR，并通过 `hvigorConfig.includeNode()` 把 library HAR 和 Registry HAR 加入构建图。目标 HAP 加载完成后，plugin 再通过 HAP model API 注入 Registry 依赖、生成资源目录与 `module.appStartup`。整个过程不会修改应用的 `src` 目录或已提交的 `build-profile.json5`。
