...
在 Lynxtron 宿主侧 Rspack 配置中使用 `pluginLynxtron()`。这个插件默认包含 Lynxtron Autolink：它会扫描已安装依赖中的 `lynx.lib.json`，选择匹配的 `platforms.lynxtron.binaries` 和 `frameworks`，把对应原生库 package 收集到宿主输出目录，并注入前置代码来加载所收集 package 的 `./lynxtron` 入口。业务代码直接导入 `<package>/lynxtron` 时，也会被别名到生成的代理模块，并在运行时解析已收集的 package。`./lynxtron` 入口负责加载当前宿主对应的 `.node` 产物。
...
对于 Lynxtron，使用 `binaries` 和 `frameworks` 明确描述运行时产物。两个字段都是数组；每项都包含 `os`、`arch`，以及相对于包根目录的 `path`（也可以是路径数组）。Autolink 会选择并校验与目标宿主匹配的项、暂存对应 package，然后加载 package 的 `./lynxtron` export。该入口读取同一份清单并加载选中的 `.node`。`binaries` 描述可以直接加载的 `.node`、`.dylib`、`.dll` 等原生文件；`frameworks` 描述 macOS Framework payload 目录，其中也可以包含 Helper app bundle，并且必须保留内部符号链接。

这些路径只标识 npm 包内的源产物。开发输出或最终应用包中的目标位置由宿主应用接入层决定。Lynxtron 的这部分清单格式仍处于 prerelease 阶段，因此不提供不透明的 package 根目录或旧字段别名：请严格使用 `binaries`、`frameworks` 和 `arch` 选择器。
