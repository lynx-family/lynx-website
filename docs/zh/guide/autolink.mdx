...
Lynx 原生库是一个 npm 包，可以打包元件、原生模块和 Service 中的任意组合，供宿主 Lynx 应用消费。Autolink 是一种集成机制，让宿主应用自动发现 `node_modules` 中安装的库，并在 Android、iOS 和 HarmonyOS 上自动注册它们的能力，避免为每个元件、原生模块或 Service 手动写注册代码。

Autolink 只提供应用级全局注册，不生成按 `LynxView` 注册 Library 的接口。
...
应用侧像安装普通 npm 依赖一样安装库。Android Gradle plugin、iOS CocoaPods plugin 和 HarmonyOS Hvigor plugin 会读取 `lynx.lib.json`，把原生代码链接进宿主应用；宿主应用本身无需了解每个库具体暴露了哪些能力。
...
- HarmonyOS 接入需要带有 `hvigorfile.ts` 的 entry 或 feature HAP 模块，并在 HAP 中依赖 `@lynx/lynx`。
...
在 HarmonyOS 工程根目录的 `hvigor/hvigor-config.json5` 中加入 plugin 依赖：
...
在 HarmonyOS 工程根目录创建 `hvigorconfig.ts`，并启用一次：

```ts
import { hvigor, hvigorConfig } from '@ohos/hvigor';
import { enableHarmonyLynxAutolink } from '@lynx/lynx-library-plugin';

enableHarmonyLynxAutolink(hvigorConfig, hvigor, {
  moduleName: 'entry',
});
```

当工程中只有一个依赖 `@lynx/lynx` 的 entry 或 feature HAP 时，可以省略 `moduleName`；存在多个 Lynx HAP 时必须明确指定目标模块。

Hvigor 创建模块节点前，plugin 会扫描当前工程可见的 `node_modules`，在工程已忽略的 `.hvigor/lynx-autolink/<moduleName>` 缓存目录中生成 Registry HAR，并通过 `hvigorConfig.includeNode()` 把 library HAR 和 Registry HAR 加入构建图。目标 HAP 加载完成后，plugin 再通过 HAP model API 注入 Registry 依赖、生成资源目录与 `module.appStartup`。AppStartup 源码保存在 HAP 模块的 `build/generated/lynx-autolink` 下；plugin 会注册一个 Hvigor 任务，在 `clean` 之后、当前 target 的 `PreBuild` 之前重新生成这些文件。整个过程不会修改应用的 `src` 目录或已提交的 `build-profile.json5`。

生成的 AppStartup 任务在主线程执行，并在应用创建 Lynx Runtime 前只调用 Registry HAR 中幂等的 `setupGlobal()`。如果 HAP 已经配置 AppStartup，plugin 会保留原有任务和 `configEntry`，再追加 Lynx 任务。应用不需要执行 `lynx-harmony-autolink` 之类的命令，也不需要手动调用 `setupGlobal()`。
...
Android 侧必须声明 `platforms.android.packageName`，`sourceDir` 默认是 `android`。iOS 侧 `sourceDir` 默认是 `ios`，`podspecPath` 默认使用 iOS 源码目录下找到的第一个 `.podspec` 文件。HarmonyOS 侧 `packageDir` 默认是 `harmony`；该目录必须是完整的源码 HAR，包含 `oh-package.json5`、`build-profile.json5`、`src/main/module.json5`，以及 `main` 字段指定的入口文件或 `Index.ets`。

安装或更新库后，Android 或 HarmonyOS 侧重新 sync/build 应用，iOS 侧重新执行 `pod install`，让生成的 registry 和原生依赖刷新。应用中不需要为每个库再写手动注册代码。
...
- `lynx.lib.json` 是 Autolink 清单文件，用来告诉宿主应用各平台原生 package 的位置。
...
- `android/`、`ios/` 和 `harmony/` 放置原生实现以及生成的原生 spec。HarmonyOS 目录是完整的 HAR package，而不是松散源码目录。
...
- 用于 Android、iOS 和 HarmonyOS Autolink 发现的 `lynx.lib.json`
- `types/index.d.ts`，在库暴露原生模块时用于声明类型
- JavaScript facade 入口 `src/index.ts`
- 原生源码目录 `android/`、`ios/`，以及完整的 `harmony/` HAR
...
- HarmonyOS：`<packageDir>/src/main/ets/generated/<ModuleName>Spec.ets`
...
HarmonyOS Library 通过一个 Provider 注册全部能力。`Index.ets` 必须导出名为 `LynxLibraryProviderImpl` 的类：
...
Native Module 继承 Codegen 生成的 ArkTS Spec：
...
包级原生初始化可以通过 `registerInitializer(() => packageInit())` 接入，例如调用原生 XElement 注册 API。原生 XElement 继续使用已有的 C++ 注册链路，不需要为了接入 Autolink 再封装成 `UIBase` 实现。

Registry 会拒绝重复的全局 Element 标签、Native Module 名称和 Service 类型，并在错误信息中标明冲突的库。全局 Native Module 会先于 `LynxView` 或 `LynxBackgroundRuntime` 显式传入的 Module map 注册，因此现有的局部 Module 仍然可以覆盖同名全局 Module。
...
Autolink 在构建期运行。它会扫描 `node_modules` 中 npm 包的 `lynx.lib.json` manifest，并为当前应用生成 registry。Android 侧由 Gradle settings plugin 和 build plugin 生成 registry；iOS 侧由 CocoaPods plugin 生成 registry pod；HarmonyOS 侧在 Hvigor 配置阶段生成源码 Registry HAR 和 AppStartup 任务。宿主应用在启动阶段只加载一次生成的 registry，因此宿主代码中不需要任何逐库接入逻辑。
