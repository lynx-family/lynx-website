...
Lynx 原生库是一个 npm 包，可以打包元件、原生模块和 Service 中的任意组合，供宿主 Lynx 应用消费。在此之前，这些能力只能在每个宿主应用里各接一次，Android 和 iOS 还要分别写一遍。Autolink 是配套的集成机制，让宿主应用自动发现 `node_modules` 中安装的库，并接入它们的原生能力，避免为每个元件、原生模块或 Service 手动写注册代码。

Autolink 在 Android 和 iOS 上提供应用级全局注册，也覆盖 Lynxtron 原生库使用的包元数据和宿主加载，但不会生成单个 `LynxView` 的局部 Library 注册，也不会生成 Web 或 HarmonyOS 的接入代码。
...
Android 和 iOS 都只会在初始化阶段加载一次生成的 registry，因此宿主代码中不需要任何逐库的接入逻辑。
