...
下文的 `targets` 清单和原生产物自动打包需要相互兼容的 `create-lynxtron`、`@lynx-js/lynxtron`、`@lynx-js/lynxtron-dev-plugins` 和 `@lynx-js/lynxtron-builder`。库作者还需要配套的 `create-lynx-library` 和 `@lynx-js/autolink-codegen`。代码合入不代表已发布的 npm 包同步更新。验证预发布版本时，请固定配套版本或本地包归档并保留锁文件，不要默认 `latest` 已包含全部修复。
