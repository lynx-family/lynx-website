...
应用侧像安装普通 npm 依赖一样安装库。Android Gradle plugin 和 iOS CocoaPods plugin 会读取 `lynx.lib.json`，把原生代码链接进宿主应用；宿主应用本身无需了解每个库具体暴露了哪些能力。Lynxtron 会从已安装包中读取同一份 manifest，stage 匹配到的包产物，并在 host bundle 中注入代码 require 每个包的 `./lynxtron` 入口。
...
- Lynxtron 接入需要应用具备 host 侧 Rspack 构建，并通过 `pluginLynxtron()` 安装和加载原生库。
...
在 Lynxtron host 侧 Rspack 配置中使用 `pluginLynxtron()`。这个插件默认包含 Lynxtron Autolink：它会扫描已安装依赖中的 `lynx.lib.json`，stage 匹配到的 native library 包，并注入代码 require 每个包的 `./lynxtron` 入口。这个入口负责加载当前 host 对应的 `.node` 产物。
...
对于 Lynxtron，`platforms.lynxtron.path` 指向 Autolink 会随库包一起 stage 的产物目录，通常是 `dist`。`platforms.macos.sourceDir` 和 `platforms.windows.sourceDir` 描述供源码型工具使用的共享 C/C++ 源码目录。库需要在 `package.json` 中导出 `./lynxtron`；这个入口负责加载当前 host 对应的 `.node` 文件，例如 `dist/macos/arm64/lynx-button.node` 或 `dist/windows/x64/lynx-button.node`。需要为 `./lynxtron` 入口支持的每个 OS 和架构分别构建并发布对应原生产物。
...
- `lynx.lib.json` 是 Autolink 清单文件，用来告诉宿主应用 Android 和 iOS 源码在哪里，以及 Lynxtron native loading 入口在哪里。
...
如果同一个包也发布 Lynxtron 原生产物，可以包含共享原生源码、Lynxtron loading 入口和分 host 输出：
...
Lynxtron native library 使用同一套 Lynx 静态注册模型。包可以暴露原生模块、元件，或同时暴露两者，不需要新增 Lynxtron 专用注册 API。

`package.json` 需要导出 Lynxtron loading 入口：
...
典型的 `lynxtron/index.cjs` 会加载当前 host 对应的产物：
...
- `types/index.d.ts` 和 `types/napi-native-module.d.ts`，在库暴露原生模块时用于声明类型
- JavaScript facade 入口 `src/index.ts`
- 原生源码目录 `android/` 和 `ios/`，以及生成 Lynxtron 产物时使用的 `shared/` native source 和 `lynxtron/` loading 入口
...
- Lynxtron N-API native module：`shared/nativeModule/` 下的 C++ N-API callback stub
