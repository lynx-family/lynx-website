...
对于 Lynxtron，使用 `targets` 按宿主组织运行时产物。每个 target 都包含 `os`、`arch`，以及名为 `binaries` 和 `frameworks` 的字符串数组中的至少一个；其中每条路径都相对于包根目录。Autolink 会选择并校验与目标宿主匹配的 target、暂存对应 package，然后加载 package 的 `./lynxtron` export。该入口读取同一份清单并加载选中的 `.node`。`binaries` 描述可以直接加载的 `.node`、`.dylib`、`.dll` 等原生文件；`frameworks` 描述 macOS Framework payload 目录，其中也可以包含 Helper app bundle，并且必须保留内部符号链接。
...
这些路径只标识 npm 包内的源产物。Lynxtron 的开发插件和 builder 会自动决定它们在开发输出及最终应用包中的位置；应用团队不需要复制这些产物，也不需要为每个库增加打包规则。Lynxtron 的这部分清单格式仍处于 prerelease 阶段，因此不提供不透明的 package 根目录或旧字段别名：请严格使用 `targets`、`binaries`、`frameworks` 和 `arch` 选择器。
...
- `lynx.lib.json` 是 Autolink 清单文件，用来声明按平台选择的 binary 或 Framework 源产物。
