...
这项能力面向常见的模块加载和执行场景，目前仅支持后台线程。目前 Lynx 并未暴露完整的 WebAssembly JavaScript API。支持范围和当前限制请参考 [WebAssembly](/zh/guide/scripting-runtime/webassembly.html)。
...
- 经过 [alias](/rspeedy/config/source#source-alias) 处理的路径：`@common/foo.js`
