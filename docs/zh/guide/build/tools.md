...
你很少需要手动引入它：[`pluginReactLynx`](/api/packages/react-rsbuild-plugin/functions/pluginReactLynx) 在它尚未注册时会自动应用，所以 ReactLynx 模板的配置里只写了 `pluginReactLynx`。
...
- **[`source.entry`](https://rsbuild.rs/config/source/entry) 用对象形式**，和其他 Rsbuild 项目一致。
- **需要配置选项时要自己引入 `pluginLynx`。** 自动应用用的是默认选项，所以想配 [`output.filename.bundle`](/api/build/output/filename/bundle) 或 [`performance.profile`](/api/build/performance/profile)，需要在 `plugins` 里显式写上 `pluginLynx({ ... })`，不会重复应用。
...
代价是能力范围：Rspeedy 的配置是 Rsbuild 配置的一个精选子集，[`tools.postcss`](https://rsbuild.rs/config/tools/postcss)、[`tools.sass`](https://rsbuild.rs/plugins/list/plugin-sass) 这类选项会被拒绝而不是透传，[`output.charset`](https://rsbuild.rs/config/output/charset)、[`output.polyfill`](https://rsbuild.rs/config/output/polyfill) 等少数值是固定的。反过来，Lynx 特有的配置直接写在配置里：[`output.filename.bundle`](/api/build/output/filename/bundle)、[`performance.profile`](/api/build/performance/profile) 不用绕 `pluginLynx` 的选项。[`source.entry`](https://rsbuild.rs/config/source/entry) 和 [`output.filename`](https://rsbuild.rs/config/output/filename) 还额外接受字符串写法。
...
- **[`source.entry`](https://rsbuild.rs/config/source/entry) 和 [`output.filename`](https://rsbuild.rs/config/output/filename) 要写成对象形式。** Rspeedy 额外接受字符串，Rsbuild 不接受。
- **Lynx 自己的选项挪进 `pluginLynx`。** [`output.filename.bundle`](/api/build/output/filename/bundle) 和 [`performance.profile`](/api/build/performance/profile) 是 Rspeedy 的配置项而非 Rsbuild 的，需要写进 `pluginLynx({ ... })`——这也意味着要显式应用这个插件，因为自动应用用的是默认选项。
