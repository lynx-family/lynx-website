...
反解需要的东西都在每次构建产出的同一个 `debug-metadata.json` 里：source map、字节码调试信息、UI source map、构建信息。它由 [`@lynx-js/debug-metadata-rsbuild-plugin`](https://www.npmjs.com/package/@lynx-js/debug-metadata-rsbuild-plugin) 产出，并由 [`pluginLynx`](https://www.npmjs.com/package/@lynx-js/rsbuild-plugin) 自动注册（默认启用，无需在配置中声明）。
...
把它接进 `rsbuild.config.ts`。你只需原样上传文件，后端用来索引的 `release` 是构建注入、并嵌在文件里的，不用你在这里指定：
...
要让 devtools 或反解服务自动 fetch 到文件，你得在模板被编码之前把 URL 写进去。这意味着要 tap `LynxTemplatePlugin.beforeEncode`，而它通过 `pluginReactLynx` 这类框架插件发布的 exposure 拿到：
