...
- 面向 Lynx 的默认值，比如默认入口、关闭代码分包、内联脚本、不注入 polyfill
...
Rspeedy 仍在维护。如果现有工具链依赖 `rspeedy` 命令行或 `lynx.config.ts`，或者你需要它更严格的配置校验和现成的默认值，继续用它没有问题。

```bash
npm create @lynx-js/lynx@latest
```

这个命令会让你选择构建工具——Rsbuild + `pluginLynx`（推荐）、Rspeedy，或用于组件库的 Rslib。
