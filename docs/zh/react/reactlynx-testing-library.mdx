...
配置 [Rstest](https://rstest.rs/) 时，可以直接复用 `lynx.config.ts` 中的配置，并指定 ReactLynx Testing Library 内置的 `setupFiles`。
...
配置 [Vitest](https://vitest.dev/) 时需要使用 `@lynx-js/react/testing-library/plugins` 中的 `vitestTestingLibraryPlugin` 插件。
