...
[`@lynx-js/react/testing-library`](/zh/react/api/testing-library) 包适用于对 ReactLynx 组件的渲染结果进行测试。它提供了和 [React Testing Library](https://testing-library.com/docs/react-testing-library/intro/) 相同的 API，例如 [`render`](/zh/react/api/testing-library#render)、[`fireEvent`](/zh/react/api/testing-library#fireevent)、[`screen`](/zh/react/api/testing-library#screen) 等，其底层使用了 [`@lynx-js/testing-environment`](/zh/packages/testing-environment) 包来提供 Lynx 环境的 JS 实现，屏蔽了 Lynx 双线程的实现细节。
...
[`render`](/zh/react/api/testing-library#render) 方法用于渲染一个 ReactLynx 组件，并返回一个 [`RenderResult`](/zh/react/api/testing-library#renderresult) 对象，其中的 `container` 字段是一个 [`LynxElement`](/zh/packages/testing-environment#lynxelement)，表示渲染结果的根元素。
...
在使用 [`fireEvent`](/zh/react/api/testing-library#fireevent) 触发事件时，需要显式指定事件的类型。例如 `new Event('catchEvent:tap')`（`eventType:eventName`） 表示触发 `catch` 类型的 `tap` 事件，请参考[事件处理器属性](/guide/interaction/event-handling/event-propagation.html#事件处理器属性)。`eventType` 的可能值和使用场景如下：
...
在 `render` 过程中，事件处理器会被挂载到 [`LynxElement`](/zh/packages/testing-environment#lynxelement) 的 [`eventMap`](/zh/packages/testing-environment#lynxelement) 属性上，因此可以通过 `eventMap` 属性来获取元素的事件处理器，用于断言事件处理器是否被正确挂载。
...
你可以使用 [`screen`](/zh/react/api/testing-library#screen) 对象来查询页面元素，它提供了一些常用的方法，例如 [`getByText`](/zh/react/api/testing-library#getbytext)、[`getByTestId`](/zh/react/api/testing-library#getbytestid) 等。还有像 [`waitForElementToBeRemoved`](/zh/react/api/testing-library#waitforelementtoberemoved) 这样的方法对页面元素状态进行等待。
...
由于 `list` 下的 `list-item` 元素是懒加载的，只有元素进入视口时才会被加载，离开视口时会被标记为可回收，在测试框架中可以使用 [`elementTree.enterListItemAtIndex`](/zh/packages/testing-environment#initelementtree) 和 [`elementTree.leaveListItem`](/zh/packages/testing-environment#initelementtree) 方法来模拟列表项元素的加载和回收。
...
[`render`](/zh/react/api/testing-library#render) 方法的第二个参数 [`RenderOptions`](/zh/react/api/testing-library#renderoptions) 中支持 [`enableMainThread`](/zh/react/api/testing-library#renderoptions) 和 [`enableBackgroundThread`](/zh/react/api/testing-library#renderoptions) 两个选项，`enableMainThread` 用于开启[首帧直出](../guide/interaction/ifr.mdx)，`enableBackgroundThread` 用于开启后台线程渲染。
...
详见 [API 参考](/zh/react/api/testing-library)。
