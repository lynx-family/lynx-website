---
description: '使用 Testing Library 风格 API 测试组件。'
---

# 代码测试

[`@lynx-js/react/testing-library`](/zh/api/react/testing-library) 包适用于对 ReactLynx 组件的渲染结果进行测试。它提供了和 [React Testing Library](https://testing-library.com/docs/react-testing-library/intro/) 相同的 API，例如 [`render`](/zh/api/react/testing-library#render)、[`fireEvent`](/zh/api/react/testing-library#fireevent)、[`screen`](/zh/api/react/testing-library#screen) 等，其底层使用了 [`@lynx-js/testing-environment`](/zh/api/packages/testing-environment) 包来提供 Lynx 环境的 JS 实现，屏蔽了 Lynx 双线程的实现细节。

## 配置

### 从 `@lynx-js/create-lynx` 创建新项目

使用 [`@lynx-js/create-lynx`](https://www.npmjs.com/package/@lynx-js/create-lynx) 创建的项目已经配置好了 Rstest 和 ReactLynx Testing Library，执行 `npm run test` 即可运行测试。

### 在已有项目中配置

ReactLynx Testing Library 集成在 `@lynx-js/react` 包的 `testing-library` 子目录中，无需额外安装其他包。目前我们支持测试框架有 Rstest 和 Vitest。

#### Rstest

使用 `@lynx-js/react/testing-library/rstest-config` 配置 [Rstest](https://rstest.rs/)。

对于库项目，推荐使用 `withDefaultConfig` 配置 Rstest。
...
对于应用项目，推荐使用 `withLynxConfig` 配置 Rstest，这样可以复用应用项目的 Lynx 配置（`lynx.config.ts`）。
...
`withLynxConfig` 和 `withDefaultConfig` 的区别是：

- `withLynxConfig`: 应用项目推荐使用，这样可以复用应用项目的 Lynx 配置（`lynx.config.ts`）。
- `withDefaultConfig`: 库项目推荐使用，极小配置，只负责注入 ReactLynx Testing Library 的默认配置（`jsdom`、setup 文件、`globals`）并允许你提供其他配置。

这样你就可以开始编写测试用例并使用 Rstest 来运行它们了。

#### Vitest

配置 [Vitest](https://vitest.dev/)：
...
这样你就可以开始编写测试用例并使用 Vitest 来运行它们了。

## 示例

:::info
下文的代码示例均用 Vitest 实现，但是你也可以在 Rstest 中使用它们。将代码示例中的 `vitest` 替换成 `@rstest/core`，并将 `vi` 替换成 `rstest` 即可在 Rstest 下使用。
:::
...
[`render`](/zh/api/react/testing-library#render) 方法用于渲染一个 ReactLynx 组件，并返回一个 [`RenderResult`](/zh/api/react/testing-library#renderresult) 对象，其中的 `container` 字段是一个 [`LynxElement`](/zh/api/packages/testing-environment#lynxelement)，表示渲染结果的根元素。
...
在使用 [`fireEvent`](/zh/api/react/testing-library#fireevent) 触发事件时，需要显式指定事件的类型。例如 `new Event('catchEvent:tap')`（`eventType:eventName`） 表示触发 `catch` 类型的 `tap` 事件，请参考[事件处理器属性](/guide/interaction/event-handling/event-propagation.html#事件处理器属性)。`eventType` 的可能值和使用场景如下：
...
在 `render` 过程中，事件处理器会被挂载到 [`LynxElement`](/zh/api/packages/testing-environment#lynxelement) 的 [`eventMap`](/zh/api/packages/testing-environment#lynxelement) 属性上，因此可以通过 `eventMap` 属性来获取元素的事件处理器，用于断言事件处理器是否被正确挂载。
...
你可以使用 [`screen`](/zh/api/react/testing-library#screen) 对象来查询页面元素，它提供了一些常用的方法，例如 [`getByText`](/zh/api/react/testing-library#getbytext)、[`getByTestId`](/zh/api/react/testing-library#getbytestid) 等。还有像 [`waitForElementToBeRemoved`](/zh/api/react/testing-library#waitforelementtoberemoved) 这样的方法对页面元素状态进行等待。
...
由于 `list` 下的 `list-item` 元素是懒加载的，只有元素进入视口时才会被加载，离开视口时会被标记为可回收，在测试框架中可以使用 [`elementTree.enterListItemAtIndex`](/zh/api/packages/testing-environment#initelementtree) 和 [`elementTree.leaveListItem`](/zh/api/packages/testing-environment#initelementtree) 方法来模拟列表项元素的加载和回收。
...
[`render`](/zh/api/react/testing-library#render) 方法的第二个参数 [`RenderOptions`](/zh/api/react/testing-library#renderoptions) 中支持 [`enableMainThread`](/zh/api/react/testing-library#renderoptions) 和 [`enableBackgroundThread`](/zh/api/react/testing-library#renderoptions) 两个选项，`enableMainThread` 用于开启[首帧直出](../guide/interaction/ifr.mdx)，`enableBackgroundThread` 用于开启后台线程渲染。
...
详见 [API 参考](/zh/api/react/testing-library)。
