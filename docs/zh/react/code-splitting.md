...
默认取值跟随 [`engineVersion`](/rspeedy/plugins/plugin-react#engineversion)：`3.9` 及以上为 `FetchBundle`，否则为 `QueryComponent`。`engineVersion` 默认是 `3.2`，因此未把它调高到 `3.9` 的构建会使用 `QueryComponent`。
...
[`experimental_isLazyBundle`]: /zh/rspeedy/plugins/plugin-react#experimental-islazybundle
