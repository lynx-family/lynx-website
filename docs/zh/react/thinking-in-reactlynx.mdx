...
2. **后台线程**运行完整的 React 运行时，负责处理组件的生命周期和其他副作用。由于无法完全与单线程 React 保持一致，我们有一个修改后的组件生命周期，详见[组件生命周期](./lifecycle.mdx)。
...
[`'background only'`]: /zh/api/react/Documents/Directives
