...
ReactLynx 本身是“地道”的 React，但也开创性地引入了“双线程 React”、编译协同渲染等优化。
...
- **[双线程编程思想](./thinking-in-reactlynx.mdx)**：ReactLynx 遵循 React 的编程模型，但通过利用 Lynx 提供的**双线程运行时**，结合自身的编程范式（或规则）来实现更好的性能和用户体验。
  ...
  - **[双线程架构下的生命周期](./lifecycle.mdx)**：由于 Lynx 的双线程架构特点，ReactLynx 的生命周期与传统 React 在调用时机上存在一些差异。
- **[编译协同渲染](./compiler-informed-rendering.mdx)**：ReactLynx 会在编译时将 JSX 的静态结构提取为可复用的编译表示，让运行时专注处理动态值和动态子树。
...
有关 ReactLynx 实现的 API 的完整列表，请参阅 [`@lynx-js/react` API 文档](/react/api/)。
...
对于 Web 上常用的 `e.stopPropagation()` 和 `e.preventDefault()` 等方法，Lynx 仅在[主线程脚本](./main-thread-script.html)中支持 [`e.stopPropagation()`](/api/lynx-api/event/event.html#stoppropagation)。
...
继续阅读[编译协同渲染](/react/compiler-informed-rendering)和[主线程脚本](/react/main-thread-script)，了解框架的渲染机制和主线程交互。
...
<NextSteps.Root>
  <NextSteps.Step
    ...
    title="开发指南"
    description="了解编码建议、代码拆分、React Compiler 和代码测试"
  />
  <NextSteps.Step
    href="/react/data-fetching"
    title="生态集成"
    description="接入数据获取、状态管理、路由和生成式 UI 等生态方案"
  />
</NextSteps.Root>

**更多内容**：按需查阅 [API 参考](/react/api/)，并使用[框架性能分析](/guide/performance/analysis-performance/analysis-framework)定位性能问题。
