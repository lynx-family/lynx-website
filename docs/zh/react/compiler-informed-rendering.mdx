...
后台首轮协调也会生成与主线程模板实例树对应的后台模板实例树。在[首帧渲染（First-frame Rendering）](/guide/spec.html#first-frame-rendering)期间，hydration 会建立两侧模板实例的对应关系，使后续更新可以用 `instanceId` 定位首帧时创建的主线程模板实例。
