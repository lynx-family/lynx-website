...
编译器负责生成可复用的模板，运行时负责执行模板中的创建和更新逻辑。在 ReactLynx 的双线程架构中，同一份模板同时用于[首帧直出（IFR）](/guide/interaction/ifr)和后续更新。

同一份模板可以被多个挂载位置复用，但每次挂载都有自己的一组模板实例：主线程实例持有创建出的 Lynx 元件，后台线程实例保存参与协调的状态和动态数据。首次同步会建立两侧实例之间的对应关系，供后续更新使用。

### 首屏：创建并关联模板实例

首屏时，主线程调用模板的 `create()` 和 `update()`，直接创建静态宿主结构并写入初始动态属性，再将动态子节点挂载到预留插槽。后台线程同时为这次渲染建立对应的实例。下面的伪代码只表示两侧的职责和对应关系，不表示实际执行顺序：
...
这里的 `Template` 表示任意编译生成的模板，其 `create()` 和 `update()` 具有上一节展示的概念形态。`connectInstances()` 概括了首次同步过程：两侧不会共享同一个实例，而是建立对应关系，让后续来自后台线程的变化可以应用到首屏已经创建的主线程实例上。

### 更新：只传递动态变化

对应关系建立后，后台线程比较动态数据：动态属性的变化可以交给编译器生成的定向更新函数，动态子节点则仍由 React 协调。
