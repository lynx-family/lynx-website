...
加载页面时，主线程直接渲染首屏内容，以实现[首帧直出（IFR）](/guide/interaction/ifr)。在 Lynx 规范中，这一过程称为[首帧渲染（First-frame Rendering，FFR）](/guide/spec.html#first-frame-rendering-or-ffr)，是[主线程渲染（Main Thread Rendering）](/guide/spec.html#main-thread-rendering)的一种。与此同时，后台线程会并行执行一次渲染；首次同步会比较两侧的模板实例树并同步差异，为后续更新做好准备。
...
```js
// 主线程：执行 React，并通过首帧渲染（FFR）完成首帧直出（IFR）
