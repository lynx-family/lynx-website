...
和 [Chunk Splitting](/config/#performance-chunksplit) 的区别可以简单理解为：External Bundle 解决跨应用复用，Chunk Splitting 解决单应用内拆包。

整个流程分成两步：

1. 用 [`@lynx-js/lynx-bundle-rslib-config`](/packages/lynx-bundle-rslib-config) 构建 external bundle。
2. 用 [`@lynx-js/external-bundle-rsbuild-plugin`](/packages/external-bundle-rsbuild-plugin) 在宿主应用里加载它。
