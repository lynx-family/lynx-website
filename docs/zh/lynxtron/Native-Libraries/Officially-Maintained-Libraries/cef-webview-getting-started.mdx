...
当前共享 CEF 存储同时只能由一个宿主进程使用。运行示例前，请关闭其他 WebView 宿主，包括之前启动的开发态或已安装应用。同一宿主支持多个 WebView。`initialize()` 失败时会抛出异常，不要忽略错误后继续创建窗口。参见[初始化限制](/lynxtron/api/@lynx-js/cef-webview/#初始化结果与进程限制)。
