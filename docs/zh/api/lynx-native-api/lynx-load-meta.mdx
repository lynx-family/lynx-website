---
api: lynx-native-api/lynx-load-meta
---
...
使用默认构造函数创建 `LynxLoadMeta`，再通过 setter 配置：
...
Desktop public API 的模板来源优先级与现有页面保持一致：
...
lynx_load_meta_set_url(meta, url);
lynx_load_meta_set_binary_data(meta, bytes, length, NULL, NULL);
lynx_load_meta_set_template_bundle(meta, bundle);
lynx_load_meta_set_initial_data(meta, initial_data);
lynx_load_meta_set_global_props(meta, global_props);
```

对象不再使用时，通过 `lynx_load_meta_release(meta)` 释放。
