...
// 在 Lynx 3.3 版本后，Animate API 可以通过 createSelectorQuery API 来调用，并且可以同时在一个节点上启动多个动画，使用方式如下：
// 该方式属于实验性功能。
...
- Animate Api 在 Lynx 3.3 版本之前仅支持创建一个动画，因此在 Lynx 3.3 版本前若对一个节点多次调用 `lynx.getElementById("test").animate`，最后创建的动画会覆盖前面的动画。

- 一个 `Animation` 对象对应的动画结束后，若想重启一个同样的动画，需要重新调用 `lynx.getElementById("test").animate` 创建一个新的 `Animation` 对象。

- Animate API Lynx 3.3 版本之前只支持 getElementById API，在 Lynx 3.3 版本后支持了 SelectQuery API.
