...
- **LynxView 容器状态问题**：宽高为 0、不在视口内、或未添加到窗口。
- **业务逻辑未渲染内容**：例如数据未就绪导致组件的 `render()` 函数返回 `null`。

**2. 监听注册过晚**

`LoadBundleEntry` 仅在首屏渲染完成时触发一次。建议尽早注册监听：

- 类组件在 `constructor` 中注册。
- 函数式组件在 `useMemo` 中注册。
...
<Go
  ...
  defaultFile="src/load-bundle-entry/index.tsx"
  defaultEntryFile="dist/load-bundle-entry.lynx.bundle"
  img="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/plugin/static/exmaple-load-bundle-entry.jpeg"
