---
api: lynx-api/performance-api/performance-entry/metric-actual-fmp-entry
---

import { APISummary, APITable, Deprecated, Go, RuntimeBadge } from '@lynx';

# <Deprecated/> MetricActualFmpEntry

:::warning
该事件在 Lynx SDK 3.7 版本已废弃，相关数据可从 [`PipelineEntry`](/api/lynx-api/performance-api/performance-entry/pipeline-entry) 中获取.
:::
...
你可以通过 [`__lynx_timing_flag="__lynx_timing_actual_fmp"`](/guide/performance/monitor-performance/timing-flag) 标记重要元件的渲染完成时机，从而产生该指标。
...
Lynx 在被标记的元件渲染完成时计算 ActualFmp。如果已经通过 [LynxView.setExtraTiming](/api/lynx-native-api/lynx-view/set-extra-timing) 提供了[容器](/guide/spec#container)时间戳，那么收到的 `MetricActualFmpEntry` 里会包含 `actualFmp`、`lynxActualFmp` 和 `totalActualFmp` 三项指标。否则，`MetricActualFmpEntry` 只包含 `lynxActualFmp`。补充容器时间戳后，其余指标将被重新计算。重新计算完成后，Lynx 将发送一个新的包含所有指标的 `MetricActualFmpEntry`。

<Go
  example="performance-api"
  defaultFile="src/actual-fmp-entry/index.tsx"
  defaultEntryFile="dist/actual-fmp-entry.lynx.bundle"
