...
使用 [`bindscroll`](#scroll)、[`bindscrolltoupper`](#scrolltoupper) / [`bindscrolltolower`](#scrolltolower) 等事件回调监听滚动进度的变化。
...
<AndroidOnly /> <IOSOnly /> <ClayOnly /> <HarmonyOnly /> <WebOnly />
...
当滚动区域的上/左边的 [`upperThreshold`](#upper-threshold) 规定的区域与可视区域相交时，触发该事件。
...
当滚动区域的上/左边的 [`lowerThreshold`](#lower-threshold) 规定的区域与可视区域相交时，触发该事件。
