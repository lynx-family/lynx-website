...
`<text>` 元件具有自己的布局上下文，类似于 Web 中的[行内格式化上下文](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_inline_layout/Inline_formatting_context)。与 `<view>` 不同,它不支持 `display` 等属性。如需了解更多关于 `<text>` 元件的文本样式设置,请参阅[文本与排版指南](/guide/styling/text-and-typography)。
...
需要客户端通过 [`GenericResourceFetcher`](/api/lynx-native-api/lynx-generic-resource-fetcher/fetch-resource.html) 实现对应的字体资源加载器，用来下载网络字体资源。
...
1. 客户端: 在创建 LynxView 时使用 `LynxViewBuilder.setFontScale()` 设置字体缩放大小；
   通过 `LynxView.updateFontScale()` 更新字体缩放大小。

2. 前端: 设置的 `font-size` 和 `line-height` 会根据客户端设置的 `fontScale` 等比例放大缩小。
...
### 跨 `<text>` 元件选择复制 {#crosstext-selection-and-copying}
...
<APITable query="elements/nested-text" />

### 嵌套 `<image>`

<APITable query="elements/nested-image" />

### `<inline-truncation>`

<APITable query="elements/inline-truncation" />
