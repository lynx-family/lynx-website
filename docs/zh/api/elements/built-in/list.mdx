...
**2. 设置滚动方向与布局形式**：设置属性 [`scroll-orientation`](#required-scroll-orientation) 指定布局与滚动方向，设置 [`list-type`](#required-list-type) 与 [`span-count`](#required-span-count) 指定布局形式。

**3. 配置子节点**：使用标签 `<list-item>` 作为 `<list>` 直接子节点，并且给 `<list-item>` 设置 [`item-key`](#required-item-key) 与 [`key`](#required-key)，并且需要保持一致。
...
**1. 设置布局方式与列数**: 设置布局形式 [`list-type`](#required-list-type) 为 `flow` (网格布局) 或者 `waterfall` (瀑布流布局)，并设置 [`span-count`](#required-span-count) >= 2。
...
List 相关 CSS 属性是专门为 `<list>` 组件设计的，例如 `list-main-axis-gap` 和 `list-cross-axis-gap`，可以对 `<list>` 内元素的布局和间距进行精细控制。需要注意的是，这些属性只在 `<list>` 上生效，在其他任何组件上都将被忽略。
...
控制 `<list>` 组件的布局类型，需要与 [`span-count`](#required-span-count) 配合使用。
...
当 `engineVersion` 版本小于 <VersionBadge v={3.4} />，该属性无法生效，因此不会拦截 [`scroll`](#scroll) 事件回调。当 `engineVersion` 版本大于等于 <VersionBadge v={3.4} />，如果不设置改属性，则使用默认值 200 ms。
...
该属性用于控制在数据源更新时，列表是否运行动画。当前支持插入、删除和更新动画。
...
当前 `update-animation` 只能在 `list-type` 为 `single` 或 `flow` 时生效。
...
当列表内容发生溢出时，在 Clay 上启用滚动条。Clay 的 `<list>` 和 `<scroll-view>` 支持相同的 [`scroll-bar-auto-hide`](/zh/api/elements/built-in/scroll-view#scroll-bar-auto-hide)、延迟、尺寸和颜色属性。自动隐藏默认为 `true`，延迟默认为 `1000` 毫秒；设置 `scroll-bar-auto-hide={false}` 才会在内容溢出时持续显示滚动条。
...
当列表内容发生溢出时启用滚动条。在 Clay 上，该名称是 [`enable-scrollbar`](#enable-scrollbar) 的已弃用别名，默认值为 `false`；Harmony 的默认值也是 `false`。在 iOS 上，默认值取决于实际启用的列表实现，因此当滚动条可见性很重要时，应显式设置该属性。
...
为 Web Lynx 的 `<list>` 启用滚动条。该名称仅适用于 Web，不是其他平台上 Clay `enable-scrollbar` 的别名。
...
在 Harmony 上，当 contentSize 小于自身容器大小，设置是否开启滑动效果。
...
`<list>` 中存在多个吸顶或者吸底的节点时，当吸顶或吸底的节点被其他吸顶或吸底的节点推出 `<list>` 区域后回收该节点，以解决吸顶或者吸底的节点过多导致的内存开销问题。此优化在 `engineVersion` 版本大于等于 <VersionBadge v={3.4} /> 时 默认开启。

<table rules="none" align="center" width="100%" style={{tableLayout:"fixed"}}>
  <tr style={{height: '300px'}}>
    <td style={{padding: '5px'}}>
      <center>
        <img src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/plugin/static/not-recycle-sticky-1.gif" style={{width: "100%"}}/>
        <br/>
        <span>吸顶的节点不回收，`<list-item>` 数量会不断增长</span>
      </center>
    </td>
    <td style={{padding: '5px'}}>
      <center>
        <img src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/plugin/static/recycle-sticky-1.gif" style={{width: "100%"}}/>
        <br/>
         <span>吸顶的节点回收，`<list-item>` 会被复用，数量不会增长</span>
      </center>
    </td>
  </tr>
</table>
...
| itemKey  | string  | null   | 否   | 指定要滚动到的节点的 `item-key`，如果同时设置了 `item-key` 和 `position` 参数，则优先滚动到 `item-key` 对应的节点                                                                                                                                                                                               |
...
确保 `<list>` 组件的 [`sticky`](#sticky) 属性设置为 `true`，以允许子节点吸顶，并且可以设置 [`sticky-offset`](#sticky-offset) 来确定吸顶或者吸底位置距离 `<list>` 顶部或者底部的偏移。
...
通过给 `<list-item>` 设置 [`recyclable`](#recyclable) 为 `false` 来实现该节点不回收。在这个例子中，我们给第一个 `<list-item>` 中添加了一个溢出且带背景色的 `<view>` 节点，当这个 `<list-item>` 被滑动出屏幕后，我们依然可以看到这个带背景色的 `<view>` 在屏幕上。
