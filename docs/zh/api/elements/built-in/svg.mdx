...
`<svg>` 是在标记中直接嵌入可缩放矢量图形（SVG）的基础元素，从简单图标到复杂的数据可视化，在不同屏幕尺寸与像素密度下都能无损缩放。`<svg>` 元素不会在主线程上为每个 SVG 标签创建独立的元素节点，而是将解析工作交由后台线程处理，并将整个图形作为一个原生视图进行渲染，大幅降低主线程在解析耗时和节点数量上的开销。因此它更适合高性能的静态矢量图形渲染，而非逐节点的交互操作或动画。它覆盖了 17 个最常用的 SVG 标签和超过 40 个属性。
