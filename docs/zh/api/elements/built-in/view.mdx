...
对于 `<page>` 节点，事件穿透行为由其 `event-through` 属性和页面配置 [`Config.enableEventThrough`](../../rspeedy/type-config.config.enableeventthrough.md) 共同决定。当触摸命中 `<page>` 节点区域时，只要其中任意一项为 `true`，Lynx 就不会消费平台层的触摸事件。
