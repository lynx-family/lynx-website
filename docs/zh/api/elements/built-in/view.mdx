...
对于 `<page>` 节点，事件穿透行为由其 `event-through` 属性和页面配置 [`Config.enableEventThrough`](https://lynxjs.org/zh/api/rspeedy/type-config.config.enableeventthrough) 共同决定。当触摸命中 `<page>` 节点区域时，只要其中任意一项为 `true`，Lynx 就不会消费平台层的触摸事件。该配置属性从 `@lynx-js/type-config` 4.1.0 起可用；该类型包版本下限与 LynxSDK 3.2 的运行时版本下限相互独立。
...
- **iOS**：LynxView 不响应触摸时，其 `hitTest` 返回 `nil`，Lynx 页面不会进入事件响应链。如果需要由兄弟节点响应事件，父容器的 `hitTest` 也需根据场景返回 `nil`。
