...
标记并监控渲染该元件的 [Lynx Pipeline](/guide/spec#lynx-pipeline)。当 Lynx Pipeline 中出现 [`__lynx_timing_flag`](/guide/performance/metrics/timing-flag) 变化，并且该 Lynx Pipeline 触发最终渲染时，将产生 [`PipelineEntry`](/api/lynx-api/performance-api/performance-entry/pipeline-entry) 性能事件，可通过 [`PerformanceObserver()`](/api/lynx-api/performance-api/performance-observer) 接收相关性能事件。
...
Android 上使用时需要在对应节点设置 [`flatten`](#flatten) 为 `false`。在 SDK 版本大于等于 3.5 支持 `androidEnablePixelCopy` 用 [PixelCopy](https://developer.android.com/reference/android/view/PixelCopy)方式截图，在需要截取 Video/Surface 等场景时推荐使用。
:::
