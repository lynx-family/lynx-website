...
在横屏时是否进入全屏输入模式，此模式下键盘和输入框将占据整个屏幕
...
| selectionEnd   | 数字   | 否       | –      | <AndroidOnly /> <IOSOnly /> <HarmonyOnly /> <VersionBadge>3.4</VersionBadge> | 3.4      | 选中范围的结束位置 |
| selectionStart | 数字   | 否       | –      | <AndroidOnly /> <IOSOnly /> <HarmonyOnly /> <VersionBadge>3.4</VersionBadge> | 3.4      | 选中范围的起始位置 |
| value          | 字符串 | 否       | –      | <AndroidOnly /> <IOSOnly /> <HarmonyOnly /> <VersionBadge>3.4</VersionBadge> | 3.4      | 输入内容           |
