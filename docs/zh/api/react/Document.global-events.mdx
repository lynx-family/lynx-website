...
当 [`lynx.__globalProps`](/api/lynx-api/lynx/lynx-global-props.mdx) 对象发生变化时，会触发 `onGlobalPropsChanged` 事件。
这对于需要响应全局配置（如主题、语言等）变化的组件非常有用。该事件的回调函数不会收到任何参数。
...
`onDataChanged` 事件主要用于处理那些**不直接关联UI渲染**的副作用（Side Effects），例如日志记录、数据上报分析或执行其他命令式操作。
