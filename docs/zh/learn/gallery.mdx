---
description: '教程：构建产品双列画廊页面。'
sidebar: false
---

import { Go, CodeFold, LearnTutorialTabs } from '@lynx';
...
首先，让我们看看结果！要查看应用的实际效果，请下载并安装 [LynxExplorer](/guide/start/quick-start.html#download-lynx-explorer,ios-simulator-platform=macos-arm64,explorer-platform=ios-simulator) 到你的设备，然后扫描下面生成的二维码。
...
:::info 在 Lynx 中的样式变体
Lynx 支持多种样式功能，包括全局样式、CSS 模块、内联样式、Sass、CSS 变量等！请参阅 [Rsbuild - CSS](https://rsbuild.rs/zh/guide/styling/css-usage)页面，了解如何选择最佳的样式配置。
:::
...
与用来添加点赞功能的 `bindtap` 事件类似，我们给 `<list>` 添加 [`scroll`](/api/elements/built-in/list.html#scroll) 事件，它会在 `<list>` 元件发生滚动时触发。为了让滚动条可以最快响应滚动事件，我们需要用 [`scroll-event-throttle`](/api/elements/built-in/list.html#scroll-event-throttle) 把事件节流设成 0。
...
:::details 什么是后台线程和主线程？
Lynx 的最大特点是双线程架构，你可以在[这篇文档](/guide/scripting-runtime/index.html#javascript-运行时)中找到更详细的介绍。
:::
...
我们还为你提供了一个通过实现一个高性能轮播图来深入学习主线程脚本的案例，请参考[教程：产品详情](/learn/product-detail)。
