...
const data = JSON.parse(readFileSync('./packages/lynx-compat-data/api-stats.json', 'utf-8'));
const platforms = ['android', 'ios', 'harmony', 'web_lynx', 'clay_android', 'clay_ios', 'clay_macos', 'clay_windows'];
...
bySupportCount.forEach((c, i) => { if (c > 0) console.log(`  ${i} platforms: ${c} features`); });

const total1 = features.filter(f => platforms.some(p => isSup(f, p))).length;
const total2 = features.filter(f => platforms.filter(p => isSup(f, p)).length >= 2).length;
console.log('');
...
for (const p of platforms) {
  ...
  console.log(`  ${p.padEnd(15)} | ${String(sup).padStart(5)}     | ${String(pct1).padStart(3)}%     | ${String(pct2).padStart(3)}%`);
}

// Features supported by exactly 1 platform
console.log('');
console.log('=== Features supported by EXACTLY 1 platform (platform-exclusive) ===');
...
for (const [p, apis] of Object.entries(singlePlatform).sort((a,b) => b[1].length - a[1].length)) {
  console.log(`  ${p}-only: ${apis.length} APIs`);
  totalExclusive += apis.length;
  apis.forEach(a => console.log(`    - ${a}`));
  console.log('');
}
console.log(`  Total exclusive: ${totalExclusive} / ${total1} = ${Math.round(totalExclusive/total1*100)}%`);
...
const relevant2 = features.filter(f => platforms.filter(p => isSup(f, p)).length >= 2);
const androidSup2 = relevant2.filter(f => isSup(f, 'android')).length;
const androidMiss2 = relevant2.filter(f => !isSup(f, 'android'));
console.log(`Android: ${androidSup2} / ${relevant2.length} = ${Math.round(androidSup2/relevant2.length*100)}%`);
