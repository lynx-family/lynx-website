...
#                            rspeedy is mirrored as full reference pages;
#                            genui is reduced to a generated overview page.
...
# whose sources don't ship), docs/{en,zh}/api/genui (generated single-page
# overview), and docs/{en,zh}/api/_meta.json (hand-curated sidebar nav). Review
# the diff and reconcile nav when members are added/removed.
#
...
sync_api_extractor_docs() {
  ...
  for loc in en zh; do
    ...
    for f in "$web"/*.md; do
      ...
      if [ -n "$allowed_pattern" ] && [[ ! "$bn" =~ $allowed_pattern ]]; then
        rm "$f"
        continue
      fi
      [ -f "$gen/$bn" ] || rm "$f"
...
sync_generated_overview_doc() {
  local name="$1"
  local target="$2"

  echo "::group::Sync generated $name overview into lynx-website"
  for loc in en zh; do
    local gen="$STACK/website/docs/$loc/api/index.md"
    local web="$WEBSITE/docs/$loc/api/$target"
    local out="$web/index.mdx"
    ...
    perl -0pi -e '
      $_ = "<!-- This file is generated by scripts/update-api-docs.sh. Do not edit directly. -->\n\n"
...
sync_generated_overview_doc "genui" "genui"
