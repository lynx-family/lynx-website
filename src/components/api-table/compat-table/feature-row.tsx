...
function prettierVersion(version: string) {
  if (`${+version}`.length === 1) {
    return `${version}.0`;
  }
...
function labelFromString(
  ...
  return (
    <>
      {+version > +(process.env.OSS ?? 0)
        ? version
        : prettierVersion(`${process.env.OSS}`)}
    </>
  );
}
...
function CompatCell({
  ...
  browserId: BCD.PlatformName;
  browserInfo: BCD.PlatformStatement;
  support: BCD.SupportStatement | undefined;
  showNotes: boolean;
  onToggle: () => void;
  locale: string;
