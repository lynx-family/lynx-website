// Vendored from `@lynx-js/go-web/lynx-view` (lynx-community/go-web#79).
//
// It is here rather than imported because the published `@lynx-js/go-web` does
// not expose it yet. Delete this file and the wrapper in `Go.tsx` once a release
// carrying that PR lands, and pass `autoGesture` straight to `<Go>` instead —
// the options object is identical, so the tutorials will not have to change.
//
// Keep edits upstream-first: fix it in go-web and re-copy, so the two do not
// drift.
